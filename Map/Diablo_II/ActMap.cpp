...
namespace Mapping
{
    ...
    ActMap::ActMap(const Level *level)
    {
        ...
        posX = (level->dwPosX == (DWORD)-1 ? 0 : level->dwPosX * 5);
        posY = (level->dwPosY == (DWORD)-1 ? 0 : level->dwPosY * 5);
    }
    ...
    void ActMap::FindRoomLinkageExits(ExitArray &exits) const
    {
        ...
        for (Room2 *room = level->pRoom2First; room; room = room->pRoom2Next)
        {
            ...
            for (DWORD i = 0; i < room->dwRoomsNear; i++)
            {
                ...
                bool startLeft = false, startRight = false, startTop = false;
                ...
                {
                    // Print("\u00FFc1d2boot\u00FFc3ActMap::GetExits\u00FFc0 adjecent room is out of reach - it's not even touching local room in corner!");
                    continue;
                }
                if (overlappingX > 0 && overlappingY > 0)
                {
                    // Print("\u00FFc1d2boot\u00FFc3ActMap::GetExits\u00FFc0 WTF local and adjecent rooms are overlapping (they share some points)!!!");
                    continue;
                }
    ...
    int ActMap::SpaceGetDataWide(const Point &point, bool abs) const
    {
        ...
        int val = GetMapData(point, abs)
                  /*| GetMapData(Point(point.first - 2, point.second - 2), abs)
                    ...
                    | GetMapData(Point(point.first + 2, point.second - 2), abs)
                    | GetMapData(Point(point.first - 2, point.second + 2), abs)*/
                  | GetMapData(Point(point.first - 2, point.second), abs) | GetMapData(Point(point.first + 2, point.second), abs) | GetMapData(Point(point.first, point.second - 2), abs) | GetMapData(Point(point.first, point.second + 2), abs);
        return val;
    }
    ...
    void ActMap::DumpLevel(const char *file) const
    {
        ...
        for (Room2 *room = level->pRoom2First; room; room = room->pRoom2Next)
        {
            for (PresetUnit *preset = room->pPreset; preset; preset = preset->pPresetNext)
            {
                ...
                fprintf(f, "Preset %d: at (%d, %d, relative: %d, %d) of type %ld, txtFile %ld, _1 %ld, _3 %ld, room (%ld, %ld) %s\n", i++, loc.first, loc.second, rel.first,
        ...
        for (Room2 *room = level->pRoom2First; room; room = room->pRoom2Next)
        {
            ...
            for (UnitAny *pUnit = room->pRoom1->pUnitFirst; pUnit; pUnit = pUnit->pRoomNext)
            {
                ...
                fprintf(f, "Unit %d: at (%d, %d, relative: %d, %d) of type %ld, txtFile %ld, %s, %d, %d\n",
        ...
        for (ExitArray::iterator it = exits.begin(); it != exits.end(); it++)
        {
            ...
            fprintf(f, "Exit %d: at (%d, %d, relative: %d, %d) of type %s linkage to area %s (id: %ld, tile id: %ld)\n", i++, it->Position.first, it->Position.second,
