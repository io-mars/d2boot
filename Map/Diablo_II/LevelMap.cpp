    ...
    void LevelMap::FindRoomLinkageExits(ExitArray &exits, RoomList &added) const
    {
        ...
        for (Room2 *room = level->pRoom2First; room; room = room->pRoom2Next)
        {
            ...
            for (DWORD i = 0; i < room->dwRoomsNear; i++)
            {
                ...
                {
                    // Print("\u00FFc1d2boot\u00FFc3LevelMap::GetExits\u00FFc0 adjecent room is out of reach - it's not even touching local room in corner!");
                    continue;
                }
                if (overlappingX > 0 && overlappingY > 0)
                {
                    // Print("\u00FFc1d2boot\u00FFc3LevelMap::GetExits\u00FFc0 WTF local and adjecent rooms are overlapping (they share some points)!!!");
                    continue;
                }
