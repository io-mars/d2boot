...
import { AutoBuilder } from "@/common/AutoBuild.js";

let AutoBuild = undefined;
let debug = true;
let templateFilename = "";
let buildTemplate = undefined;
...
function spendStatPoints() {
  if (!buildTemplate[me.charlvl] || !buildTemplate[me.charlvl].StatPoints)
    return false;

  let stats = buildTemplate[me.charlvl].StatPoints;
  let errorMessage =
    "\nInvalid stat point set in build template " +
    templateFilename +
    " at level " +
    me.charlvl;
  ...
  if (len > unusedStatPoints) {
    ...
    AutoBuild.print(
      "Warning: Number of stats specified in your build template at level " +
        me.charlvl +
        " exceeds the available unused stat points" +
        "\nOnly the first " +
        len +
        " stats " +
        stats.slice(0, len).join(", ") +
        " will be added"
    );
  }
  ...
  for (let i = 0; i < len; i++) {
    ...
    let statIsValid =
      typeof id === "number" &&
      sdk.stats.Strength <= id &&
      id <= sdk.stats.Vitality;
    ...
    if (id === -1) {
      ...
      throw new Error(
        "Stat id must be one of the following:\n0:" +
          STAT_ID_TO_NAME[0] +
          ",\t1:" +
          STAT_ID_TO_NAME[1] +
          ",\t2:" +
...
function getRequiredSkills(id) {
  function searchSkillTree(id) {
    let results = [];
    let skillTreeRight = getBaseStat(
      "skills",
      id,
      sdk.stats.PreviousSkillRight
    );
    let skillTreeMiddle = getBaseStat(
      "skills",
      id,
      sdk.stats.PreviousSkillMiddle
    );
    ...
    for (let i = 0; i < results.length; i++) {
      let skill = results[i];
      let skillInValidRange =
        sdk.skills.Attack < skill &&
        skill <= sdk.skills.PhoenixStrike &&
        ![
          sdk.skills.IdentifyScroll,
          sdk.skills.BookofIdentify,
          sdk.skills.TownPortalScroll,
          sdk.skills.BookofTownPortal,
        ].contains(skill);
      let hardPointsInSkill = me.getSkill(
        skill,
...
function spendSkillPoints() {
  if (!buildTemplate[me.charlvl] || !buildTemplate[me.charlvl].SkillPoints)
    return false;

  let skills = buildTemplate[me.charlvl].SkillPoints;
  let errInvalidSkill =
    "\nInvalid skill point set in build template " +
    templateFilename +
    " for level " +
    me.charlvl;
  let spentEveryPoint = true;
  ...
  if (len > unusedSkillPoints) {
    ...
    AutoBuild.print(
      "Warning: Number of skills specified in your build template at level " +
        me.charlvl +
        " exceeds the available unused skill points" +
        "\nOnly the first " +
        len +
        " skills " +
        skills.slice(0, len).join(", ") +
        " will be added"
    );
  }
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (id === -1) {
      ...
      throw new Error(
        "Skill id " +
          id +
          " is not a skill for your character class" +
          errInvalidSkill
      );
    }
    ...
    let requiredLevel = getBaseStat(
      "skills",
      id,
      sdk.stats.MinimumRequiredLevel
    );
...
async function main() {
  try {
    ...
    AutoBuild = new AutoBuilder();
    AutoBuild.print("Loaded AutoBuild thread");
    templateFilename = AutoBuild.getTemplateFilename();
    buildTemplate = await AutoBuild.loadTemplate(false);
    ...
    while (true) {
      ...
      if (
        ...
        debug &&
          AutoBuild.print(
            "Incrementing cached character level to",
            prevLevel + 1
          );
...
Config.asyncInit()
  .then(() => {
    // addEventListener("chatmsg", ({ msg }) => {
    //   // print(`--AutoBT--chatmsg----${msg}`);
    //   if (msg === "skill") {
    //     spendSkillPoints();
    //     spendStatPoints();
    //   }
