...
async function Party() {
  ...
  addEventListener("gameevent", ({ mode, param1, param2, name1, name2 }) => {
    let player;
    ...
    switch (mode) {
      case 0x02: // "%Name1(%Name2) joined our world. Diablo's minions grow stronger."
        if (Config.Greetings.length > 0) {
          ...
          if (player && player.name !== me.name) {
            say(
              Config.Greetings[rand(0, Config.Greetings.length - 1)]
                .replace("$name", player.name)
                .replace("$level", player.level)
                .replace("$class", classes[player.classid])
            );
          }
        }
      ...
      case 0x06: // "%Name1 was Slain by %Name2"
        if (Config.DeathMessages.length > 0) {
          ...
          if (player && player.name !== me.name) {
            say(
              Config.DeathMessages[rand(0, Config.DeathMessages.length - 1)]
                .replace("$name", player.name)
                .replace("$level", player.level)
                .replace("$class", classes[player.classid])
                .replace("$killer", name2)
            );
          }
        }
  ...
  print(
    "\xFFc2Party thread loaded. Mode: " +
      (Config.PublicMode === 2 ? "Accept" : "Invite")
  );
  ...
  while (true) {
    ...
    while (true) {
      if (
        me.gameReady &&
        (!Config.PartyAfterScript ||
          scriptList.indexOf(currScript) >
            scriptList.indexOf(Config.PartyAfterScript))
      ) {
        player = getParty();
          ...
          while (player.getNext()) {
            ...
            if (Config.UnpartyShitlisted) {
              ...
              if (
                ...
                shitList.indexOf(player.name) === -1
              ) {
                shitList.push(player.name);
              }
                ...
                if (player.level > playerLevels[player.name]) {
                  say(
                    Config.Congratulations[
                      rand(0, Config.Congratulations.length - 1)
                    ]
                      .replace("$name", player.name)
                      .replace("$level", player.level)
                      .replace("$class", classes[player.classid])
