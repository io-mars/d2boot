...
import { Misc, Game, Time, Packet, Sort } from "@/common/Misc.js";
...
let commandDesc = {
  ...
  go: "Rush bus",
  skiptoact: "Rush the act, skiptoact:1",
  clear: "Clear the area, clear:3, clear ColdPlains",
  highestquest: "Do quest, highestquest:cain",
};

Pather._useWaypoint = Pather.useWaypoint;

Pather.useWaypoint = function (targetArea, check) {
  if (Pather._useWaypoint(targetArea, check)) {
    return (Config.Rusher.GiveWps && giveWP()) || true;
  } else {
    print("give wp failed");

    return false;
  }
};

function wpEvent({ nick, msg }) {
  if (
    (typeof msg === "string" && msg === "gotwp") ||
    msg === "Failed to get wp"
  ) {
    count++;
    !silentNameTracker.includes(nick) && silentNameTracker.push(nick);
  }
}
...
function giveWP() {
  ...
  if (wp && !me.inTown && wpsToGive.includes(me.area)) {
    try {
      ...
      let mobCount = Game.getUnits(sdk.unittype.Monster).filter(
        (mon) => mon.distance <= 15 && mon.attackable
      ).length;

      mobCount > 0 &&
        Attack.securePosition(me.x, me.y, 15, Time.seconds(30), true);
      ...
      while (getTickCount() - tick < Time.minutes(2)) {
        ...
        if (player) {
          do {
            if (
              player.name !== me.name &&
              !silentNameTracker.includes(player.name)
            ) {
        ...
        if (
          count === playerCount ||
          (silentNameTracker.length === playerCount &&
            Misc.getNearbyPlayerCount() === 0) ||
          Config.Rusher.WaitPlayerCount <= count
        ) {
...
function RushThread() {
  ...
  this.bumperCheck = function () {
    ...
    if (!Misc.checkPartyLevel(bumperLevelReq)) {
      this.log(
        `No eligible detected, your level less: \xFFc1${bumperLevelReq}\xFFc0. Rush complete~`
      );
      return false;
    }
    return true;
  ...
  this.andariel = function () {
    ...
    if (
      !Pather.moveToExit(
        [sdk.areas.CatacombsLvl3, sdk.areas.CatacombsLvl4],
        true
      ) ||
  ...
  this.summoner = function () {
    ...
    Town.doChores();
    Pather.useWaypoint(sdk.areas.ArcaneSanctuary, true) &&
      Precast.doPrecast(true);

    let preset = Game.getPresetObject(
  ...
  this.mephisto = function () {
    ...
    Town.doChores();
    Pather.useWaypoint(sdk.areas.DuranceofHateLvl2, true) &&
      Precast.doPrecast(true);
  ...
  this.diablo = function () {
    ...
    try {
      ...
      inviteIn(this) && Common.Diablo.diabloPrep();
    } catch (error) {
  ...
  this.radament = function () {
    ...
    let moveIntoPos = function (unit, range) {
      ...
      let angles = [
        ...
        -120, 135, -135, 150, -150, 180,
      ];

      for (let i = 0; i < angles.length; i += 1) {
        let coordx = Math.round(
          Math.cos(((angle + angles[i]) * Math.PI) / 180) * range + unit.x
        );
        let coordy = Math.round(
          Math.sin(((angle + angles[i]) * Math.PI) / 180) * range + unit.y
        );
    ...
    Pather.useWaypoint(sdk.areas.A2SewersLvl2, true) &&
      Precast.doPrecast(false);
    Pather.moveToExit(sdk.areas.A2SewersLvl3, true);

    let radaPreset = Game.getPresetObject(
    ...
    let rada = Misc.poll(
      () => Game.getMonster(sdk.monsters.Radament),
      1500,
      500
    );
  ...
  this.lamesen = function () {
    ...
    if (
      !Pather.moveToExit(sdk.areas.RuinedTemple, true) ||
      !Pather.moveToPreset(
        me.area,
        sdk.unittype.Object,
  ...
  this.izual = function () {
    ...
    Pather.useWaypoint(sdk.areas.CityoftheDamned, true) &&
      Precast.doPrecast(false);
    Pather.moveToExit(sdk.areas.PlainsofDespair, true);

    let izualPreset = Game.getPresetMonster(
  ...
  this.shenk = function () {
    ...
    Pather.useWaypoint(sdk.areas.FrigidHighlands, true) &&
      Precast.doPrecast(false);
  ...
  this.anya = function () {
    ...
    if (
      !Pather.moveToExit(sdk.areas.FrozenRiver, true) ||
      !Pather.moveToPreset(
        me.area,
        sdk.unittype.Object,
        sdk.objects.FrozenAnyasPlatform
      )
    ) {
      throw new Error("Anya quest failed");
    }
  ...
  this.givewps = function () {
    ...
    wpsLeft.forEach(function (wp) {
      Town.checkScrolls(sdk.items.TomeofTownPortal) <= 5 &&
        (Pather.useWaypoint(sdk.areas.townOf(me.area)) || Town.goToTown()) &&
        Town.doChores();
  ...
  while (true) {
    if (command) {
      switch (command) {
        case "help":
          // let str = "";
          for (let cmd in commandDesc) {
            // str += cmd + " (" + commandDesc[cmd] + "), ";
            say(`\xFFc9${cmd}\xFFc0 ${commandDesc[cmd]}`);
          }
          say(`\xFFc2boss\xFFc0: ${sequence.join()}`);
        ...
        case "go":
          ...
          if (
            ...
            (Config.Rusher.LastRun &&
              current > sequence.indexOf(Config.Rusher.LastRun))
          ) {
            delay(3000);
            this.log("bye ~");
            console.log(
        ...
        default:
          if (typeof command === "string") {
            if (
              command.split(":")[0] !== undefined &&
              command.split(":")[0] === "skiptoact"
            ) {
