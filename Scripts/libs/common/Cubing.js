...
export const Cubing = {
  ...
  gemList: [],
  chippedGems: [
    sdk.items.gems.Chipped.Amethyst,
    sdk.items.gems.Chipped.Topaz,
    sdk.items.gems.Chipped.Sapphire,
    sdk.items.gems.Chipped.Emerald,
    sdk.items.gems.Chipped.Ruby,
    sdk.items.gems.Chipped.Diamond,
    sdk.items.gems.Chipped.Skull,
  ],
      ...
      if (Config.Recipes[i].length > 1 && isNaN(Config.Recipes[i][1])) {
        if (
          NTIPAliasClassID.hasOwnProperty(
            Config.Recipes[i][1].replace(/\s+/g, "").toLowerCase()
          )
        ) {
          Config.Recipes[i][1] =
            NTIPAliasClassID[
          ...
          Misc.errorReport(
            "\xFFc1Invalid cubing entry:\xFFc0 " + Config.Recipes[i][1]
          );
  ...
  buildGemList() {
    let gemList = [
      sdk.items.gems.Perfect.Amethyst,
      sdk.items.gems.Perfect.Topaz,
      sdk.items.gems.Perfect.Sapphire,
      sdk.items.gems.Perfect.Emerald,
    ...
    for (let i = 0; i < this.recipes.length; i += 1) {
      ...
      if (
        [Recipe.Gem, Recipe.Reroll.Magic].indexOf(this.recipes[i].Index) === -1
      ) {
  ...
  buildRecipes() {
    ...
    for (let i = 0; i < Config.Recipes.length; i += 1) {
      if (
        typeof Config.Recipes[i] !== "object" ||
      ...
      switch (Config.Recipes[i][0]) {
        ...
        case Recipe.HitPower.Amulet:
          this.recipes.push({
            Ingredients: [
              sdk.items.Amulet,
              sdk.items.runes.Thul,
              sdk.items.Jewel,
              sdk.items.gems.Perfect.Sapphire,
            ],
            Level: 90,
            Index: Recipe.HitPower.Amulet,
          });
        ...
        case Recipe.Reroll.Magic: // Hacky solution ftw
          // iomars only Nihlathak or baal GC
          // Level: 91,
          this.recipes.push({
            Ingredients: [Config.Recipes[i][1], "pgem", "pgem", "pgem"],
            Level: 95,
        ...
        case Recipe.LowToNorm.Weapon:
          this.recipes.push({
            Ingredients: [Config.Recipes[i][1], sdk.items.runes.Eld, "cgem"],
        ...
        case Recipe.LowToNorm.Armor:
          this.recipes.push({
            Ingredients: [Config.Recipes[i][1], sdk.items.runes.El, "cgem"],
              ...
              if (me.ladder) {
                this.recipes.push({
                  Ingredients: [
                    ...
                    sdk.items.runes.Lum,
                    sdk.items.runes.Lum,
                    sdk.items.gems.Flawed.Amethyst,
                  ],
                  Index: Recipe.Rune,
                });
              }
              ...
              if (me.ladder) {
                this.recipes.push({
                  Ingredients: [
                    sdk.items.runes.Ist,
                    sdk.items.runes.Ist,
                    sdk.items.gems.Normal.Sapphire,
                  ],
                  Index: Recipe.Rune,
                });
              }
        ...
        case Recipe.Token:
          this.recipes.push({
            Ingredients: [
              sdk.quest.item.TwistedEssenceofSuffering,
              sdk.quest.item.ChargedEssenceofHatred,
              sdk.quest.item.BurningEssenceofTerror,
              sdk.quest.item.FesteringEssenceofDestruction,
            ],
            Index: Recipe.Token,
            AlwaysEnabled: true,
          });
  ...
  buildLists() {
    ...
    for (let i = 0; i < this.recipes.length; i += 1) {
      ...
      IngredientLoop: for (
        ...
        for (let k = 0; k < items.length; k += 1) {
          if (
            ((this.recipes[i].Ingredients[j] === "pgem" &&
              this.gemList.includes(items[k].classid)) ||
              (this.recipes[i].Ingredients[j] === "cgem" &&
                this.chippedGems.includes(items[k].classid)) ||
              items[k].classid === this.recipes[i].Ingredients[j]) &&
            this.validItem(items[k], this.recipes[i])
          ) {
            ...
            if (
              this.recipes[i].Index !== Recipe.Rune ||
        ...
        this.neededIngredients.push({
          classid: this.recipes[i].Ingredients[j],
        ...
        // Make perf amethyst
        if (
          this.subRecipes.indexOf(sdk.items.gems.Perfect.Amethyst) === -1 &&
        ...
        // Make perf topaz
        if (
          this.subRecipes.indexOf(sdk.items.gems.Perfect.Topaz) === -1 &&
          (this.recipes[i].Ingredients[j] === sdk.items.gems.Perfect.Topaz ||
            (this.recipes[i].Ingredients[j] === "pgem" &&
              this.gemList.indexOf(sdk.items.gems.Perfect.Topaz) > -1))
        ) {
          this.recipes.push({
            Ingredients: [
              sdk.items.gems.Flawless.Topaz,
        ...
        // Make perf sapphire
        if (
          this.subRecipes.indexOf(sdk.items.gems.Perfect.Sapphire) === -1 &&
          (this.recipes[i].Ingredients[j] === sdk.items.gems.Perfect.Sapphire ||
            (this.recipes[i].Ingredients[j] === "pgem" &&
              this.gemList.indexOf(sdk.items.gems.Perfect.Sapphire) > -1))
        ) {
          this.recipes.push({
            Ingredients: [
              sdk.items.gems.Flawless.Sapphire,
        ...
        // Make perf emerald
        if (
          this.subRecipes.indexOf(sdk.items.gems.Perfect.Emerald) === -1 &&
          (this.recipes[i].Ingredients[j] === sdk.items.gems.Perfect.Emerald ||
            (this.recipes[i].Ingredients[j] === "pgem" &&
              this.gemList.indexOf(sdk.items.gems.Perfect.Emerald) > -1))
        ) {
          this.recipes.push({
            Ingredients: [
              sdk.items.gems.Flawless.Emerald,
        ...
        // Make perf ruby
        if (
          this.subRecipes.indexOf(sdk.items.gems.Perfect.Ruby) === -1 &&
          (this.recipes[i].Ingredients[j] === sdk.items.gems.Perfect.Ruby ||
            (this.recipes[i].Ingredients[j] === "pgem" &&
              this.gemList.indexOf(sdk.items.gems.Perfect.Ruby) > -1))
        ) {
          this.recipes.push({
            Ingredients: [
              sdk.items.gems.Flawless.Ruby,
        ...
        // Make perf diamond
        if (
          this.subRecipes.indexOf(sdk.items.gems.Perfect.Diamond) === -1 &&
          (this.recipes[i].Ingredients[j] === sdk.items.gems.Perfect.Diamond ||
            (this.recipes[i].Ingredients[j] === "pgem" &&
              this.gemList.indexOf(sdk.items.gems.Perfect.Diamond) > -1))
        ) {
          this.recipes.push({
            Ingredients: [
        ...
        // Make perf skull
        if (
          this.subRecipes.indexOf(sdk.items.gems.Perfect.Skull) === -1 &&
          (this.recipes[i].Ingredients[j] === sdk.items.gems.Perfect.Skull ||
            (this.recipes[i].Ingredients[j] === "pgem" &&
              this.gemList.indexOf(sdk.items.gems.Perfect.Skull) > -1))
        ) {
          this.recipes.push({
            Ingredients: [
  ...
  checkRecipe(recipe) {
    ...
    for (let i = 0; i < recipe.Ingredients.length; i += 1) {
      for (let j = 0; j < this.validIngredients.length; j += 1) {
        if (
          usedGids.indexOf(this.validIngredients[j].gid) === -1 &&
          (this.validIngredients[j].classid === recipe.Ingredients[i] ||
            (recipe.Ingredients[i] === "pgem" &&
              this.gemList.includes(this.validIngredients[j].classid)) ||
            (recipe.Ingredients[i] === "cgem" &&
              this.chippedGems.includes(this.validIngredients[j].classid)))
        ) {
          let item = me.getItem(
            this.validIngredients[j].classid,
            -1,
            this.validIngredients[j].gid
          );
  ...
  checkItem(unit) {
    ...
    for (let i = 0; i < this.neededIngredients.length; i += 1) {
      if (
        unit.classid === this.neededIngredients[i].classid &&
        this.validItem(unit, this.neededIngredients[i].recipe)
      ) {
        //Misc.debugLog("Cubing: " + unit.name + " " + this.neededIngredients[i].recipe.Index + " " + (this.neededIngredients[i].recipe.hasOwnProperty("MainRecipe") ? this.neededIngredients[i].recipe.MainRecipe : "") + this.getRecipeNeeds(this.neededIngredients[i].recipe.Index));
        return true;
      }
    }
  ...
  keepItem(unit) {
    ...
    for (let i = 0; i < this.validIngredients.length; i += 1) {
      if (
        unit.mode === sdk.items.mode.inStorage &&
        unit.gid === this.validIngredients[i].gid
      ) {
        return true;
      }
    }
  ...
  validItem(unit, recipe) {
    ...
    if (
      ...
      if (
        !recipe.Enabled &&
        recipe.Ingredients[0] !== unit.classid &&
        recipe.Ingredients[1] !== unit.classid
      ) {
        return false;
      }
    ...
    if (
      recipe.Index >= Recipe.HitPower.Helm &&
      recipe.Index <= Recipe.Safety.Weapon
    ) {
      ...
      if (unit.itemType === sdk.items.type.Jewel) {
        ...
        unit.magic &&
        Math.floor(me.charlvl / 2) + Math.floor(unit.ilvl / 2) >=
          recipe.Level &&
        ntipResult === Pickit.Result.UNWANTED
      ) {
        //iomars add Ethereal support
        switch (recipe.Ethereal) {
          case Roll.All:
          case undefined:
            return true;
          case Roll.Eth:
            return unit.getFlag(0x400000);
          case Roll.NonEth:
            return !unit.getFlag(0x400000);
        }
      }
    ...
    let socketNormal =
      recipe.Index >= Recipe.Socket.Shield &&
      recipe.Index <= Recipe.Socket.Helm;

    if (upgradeUnique || upgradeRare || socketNormal) {
      switch (true) {
        case upgradeUnique &&
          unit.unique &&
          ntipResult === Pickit.Result.WANTED: // Unique item matching pickit entry
    ...
    if (
      recipe.Index === Recipe.LowToNorm.Armor ||
      recipe.Index === Recipe.LowToNorm.Weapon
    ) {
  ...
  doCubing(randomize = false) {
    ...
    for (let i = 0; i < tempArray.length; i += 1) {
      ...
      if (items) {
        ...
        if (
          (!getUIFlag(sdk.uiflags.Cube) && !Town.openStash()) ||
          !this.emptyCube()
        )
          return false;
        ...
        while (items.length) {
          string +=
            items[0].name.omitColor(true) + (items.length > 1 ? " + " : "");
  ...
  makeRevPots() {
    ...
    let revpots = me
      .getItemsEx()
      .filter((item) => item.classid === sdk.items.RejuvenationPotion);
    ...
    (cubeInStash = cube.location !== locations.Inventory) &&
    ...
    revpots.forEach(function (pot, index) {
      ...
      if (!index || (1 + index) % 3 !== 0) {
        ...
        origin
          ...
          .some(function (orgin) {
            // Loop over all the original spots.
