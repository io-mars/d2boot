...
import { Skill, Packet, Misc, Game, Time } from "./Misc.js";
...
export const Town = {
  ...
  lastInteractedNPC: {
    ...
    get() {
      try {
        if (
          !!this.unit &&
          getTickCount() - this.tick < Time.seconds(15) &&
          this.unit.name.toLowerCase() !== "an evil force" &&
          this.unit.area === me.area
        ) {
  ...
  needPotions() {
    ...
    if (!Config.BeltColumn.some((el) => ["hp", "mp"].includes(el)))
      return false;
    ...
    if (me.charlvl > 2 && me.gold > 1000) {
      ...
      me.getItemsEx(-1, sdk.items.mode.inBelt)
        .filter(
          (p) =>
            [sdk.items.type.HealingPotion, sdk.items.type.ManaPotion].includes(
              p.itemType
            ) && p.x < 4
        )
        .forEach((p) => {
      ...
      // quick check
      if (
        (Config.BeltColumn.includes("hp") && !pots.hp.length) ||
  ...
  doChores(repair = false) {
    ...
    if (
      ...
      !me.getState(sdk.states.BurstofSpeed)
    ) {
  ...
  npcInteract(name = "", cancel = true) {
    ...
    switch (NPC[name]) {
      ...
      case NPC.Hratli:
        if (
          !me.getQuest(sdk.quest.id.SpokeToHratli, sdk.quest.states.Completed)
        ) {
  ...
  getTpTool() {
    let items = me
      ...
      .filter(
        (item) =>
          item.isInInventory &&
          [sdk.items.ScrollofTownPortal, sdk.items.TomeofTownPortal].includes(
            item.classid
          )
      );
    ...
    let tome = items.find(
      (i) =>
        i.classid === sdk.items.TomeofTownPortal &&
        i.getStat(sdk.stats.Quantity) > 0
    );
    if (tome) return tome;
  ...
  getIdTool() {
    let items = me
      ...
      .filter(
        (i) =>
          i.isInInventory &&
          [sdk.items.ScrollofIdentify, sdk.items.TomeofIdentify].includes(
            i.classid
          )
      );
    if (!items.length) return null;
    let tome = items.find(
      (i) =>
        i.isInInventory &&
        i.classid === sdk.items.TomeofIdentify &&
        i.getStat(sdk.stats.Quantity) > 0
    );
    if (tome) return tome;
    let scroll = items.find(
      (i) => i.isInInventory && i.classid === sdk.items.ScrollofIdentify
    );
    if (scroll) return scroll;
    return null;
  },

  canTpToTown(force = false) {
    // can't tp if dead
    ...
    //the first at ArreatSummit
    !force && badAreas.push(sdk.areas.ArreatSummit);
  ...
  initNPC(task = "", reason = "undefined") {
    ...
    let justUseClosest =
      ["clearInventory", "sell"].includes(reason) && !Town.getUnids();

    try {
      if (!!npc) {
        if (
          !justUseClosest &&
          (npc.name.toLowerCase() !== this.tasks[me.act - 1][task] ||
            // Jamella gamble fix
      ...
      if (justUseClosest) {
        ...
        npc = Game.getUnits(sdk.unittype.NPC)
          .sort((a, b) => a.distance - b.distance)
          .find((unit) =>
            [npcs.Shop, npcs.Repair].includes(unit.name.toLowerCase())
          );
      }
      ...
      if (
        !npc ||
        npc.area !== me.area ||
        (!getUIFlag(sdk.uiflags.NPCMenu) && !npc.openMenu())
      ) {
      ...
      if (!!e.message && e.message === "Couldn't interact with npc") {
        ...
        let myAct = me.act;
        let potentialActs = [1, 2, 3, 4, 5].filter(
          (a) => a <= highestAct && a !== myAct
        );
        let goTo = potentialActs[rand(0, potentialActs.length - 1)];
        Config.DebugMode &&
          console.debug(
            "Going to Act " + goTo + " to see if it fixes getUnit bug"
          );
        Town.goToTown(goTo);
      }
    ...
    if (task === "Heal") {
      ...
      if (me.getState(sdk.states.Frozen)) {
        console.log(
          "We are frozen, lets unfreeze real quick with some thawing pots"
        );
  ...
  buyPotions() {
    ...
    if (buffer.mp < Config.MPBuffer || buffer.hp < Config.HPBuffer) {
      if (
        Config.BeltColumn.some(
          (c, i) => col[i] >= beltSize && (!needPots || c === "rv")
        )
      ) {
        specialCheck = true;
      }
    ...
    buffer.mp >= Config.MPBuffer &&
      buffer.hp >= Config.HPBuffer &&
      (needBuffer = false);
    ...
    if (
      specialCheck &&
      Config.BeltColumn.some((c, i) => c === "rv" && col[i] >= beltSize)
    ) {
      let pots = [
      ...
      Config.BeltColumn.forEach((c, i) => {
        if (c === "rv" && col[i] >= beltSize && pots.length) {
          ...
          if (pot) {
            ...
            pot = me
              .getItemsEx(usePot, sdk.items.mode.inStorage)
    ...
    // re-check
    !needBuffer &&
      (Config.HPBuffer > 0 || Config.MPBuffer > 0) &&
      getNeededBuffer();
  ...
  getPotion(npc, type, highestPot = 5) {
  ...
  fillTome(classid) {
    ...
    if (
      classid === sdk.items.TomeofTownPortal &&
      !me.findItem(
  ...
  identify() {
    ...
    if (
      !list.some((item) => {
        ...
        return (
          (!identified || Config.LowGold > 0) &&
          [Pickit.Result.UNID, Pickit.Result.TRASH].includes(
            Pickit.checkItem(item).result
          ) /*  || (!identified && AutoEquip.hasTier(item)) */
        );
      })
    ) {
      return false;
    ...
    !!tome &&
      tome.getStat(sdk.stats.Quantity) < list.length &&
      this.fillTome(sdk.items.TomeofIdentify);
    ...
    MainLoop: while (list.length > 0) {
      ...
      if (
        !item.identified &&
        item.isInInventory &&
        !this.ignoredItemTypes.includes(item.itemType)
      ) {
          ...
          case Pickit.Result.UNID:
            ...
            switch (result.result) {
              ...
              default: // shop speedup test
                ...
                let timer = getTickCount() - this.sellTimer;
        ...
        switch (result.result) {
          case Pickit.Result.UNWANTED:
            ...
            if (
              ...
              Config.DroppedItemsAnnounce.Quality.includes(item.quality)
            ) {
              say(
                "Dropped: [" +
              ...
              if (
                ...
                Config.DroppedItemsAnnounce.OOGQuality.includes(item.quality)
              ) {
  ...
  identifyItem(unit, tome, packetID = false) {
  ...
  shopItems() {
    ...
    for (let i = 0; i < items.length; i += 1) {
      ...
      if (
        result.result ===
        Pickit.Result.WANTED /*  && Item.autoEquipCheck(items[i]) */
      ) {
        try {
          if (
  ...
  gamble() {
      ...
      for (let i = 0; i < Config.GambleItems.length; i += 1) {
        if (isNaN(Config.GambleItems[i])) {
          if (
            ...
            Misc.errorReport(
              "\xFFc1Invalid gamble entry:\xFFc0 " + Config.GambleItems[i]
            );
          }
        } else {
    ...
    while (me.gold >= Config.GambleGoldStop) {
      ...
      if (item) {
        ...
        //iomars more characters gambling at one npc,then return
        if (
          items.length === 0 &&
          this.gambleIds.some(
            (cid) => cid === sdk.items.Amulet || cid === sdk.items.Ring
          )
        ) {
          me.overhead(`\xFFc4Gambled:\xFFc0 should be at another time!`);
          return false;
        }
          ...
          if (newItem) {
            ...
            switch (result.result) {
              ...
              default:
                ...
                me.overhead(
                  `\xFFc4Sell:\xFFc0 ${Pickit.itemColor(newItem)}${
                    newItem.name
                  }\xFFc0`
                );
                newItem.sell();
  ...
  buyPots(
    ...
    typeof type === "string" &&
      (type = sdk.items[type.capitalize(true) + "Potion"] || false);
    if (!type) return false;
    ...
    let potDealer = ["Akara", "Lysander", "Alkor", "Jamella", "Malah"][
      me.act - 1
    ];
    ...
    switch (type) {
      ...
      case sdk.items.StaminaPotion:
        ...
        if (!force && (Skill.canUse(sdk.skills.Vigor) || Pather.canTeleport()))
          return true;
    ...
    try {
      if (
        !!npc &&
        npc.name.toLowerCase() === NPC[potDealer] &&
        !getUIFlag(sdk.uiflags.Shop)
      ) {
        if (!npc.startTrade("Shop"))
  ...
  drinkPots(type = undefined, log = true) {
    // convert to classid if isn't one
    typeof type === "string" &&
      (type = sdk.items[type.capitalize(true) + "Potion"] || false);
  ...
  cubeRepair() {
    ...
    let items = this.getItemsForRepair(Config.RepairPercent, false).sort(
      (a, b) => a.durabilityPercent - b.durabilityPercent
    );
            ...
            if (cubeItems[0].bodylocation === bodyLoc) {
              print(
                cubeItems[0].fname
                  ...
                  .reverse()
                  .join(" ")
                  .omitColor(true) + " successfully repaired and equipped."
              );
  ...
  getItemsForRepair(repairPercent, chargedItems) {
    let itemList = [];
                ...
                if (
                  ...
                  (quantity * 100) /
                    (getBaseStat("items", item.classid, "maxstack") +
                      item.getStat(sdk.stats.ExtraStack)) <=
                    repairPercent
                ) {
  ...
  reviveMerc() {
    ...
    let regex = new RegExp(
      getLocaleString(sdk.locale.dialog.youDoNotHaveEnoughGoldForThat),
      "gi"
    );
    MainLoop: for (let i = 0; i < 3; i += 1) {
      ...
      for (let lines = 0; lines < dialog.length; lines += 1) {
        ...
        if (dialog[lines].text.match(regex)) {
          return false;
        }
      }
  ...
  canStash(item) {
    ...
    return !(
      this.ignoredItemTypes.includes(item.itemType) ||
      questClassids.includes(item.classid) ||
      !Storage.Stash.CanFit(item)
    );
  },

  stash(stashGold = true) {
    ...
    if (items) {
      for (let i = 0; i < items.length; i += 1) {
        if (this.canStash(items[i])) {
          ...
          switch (true) {
            case pickResult > Pickit.Result.UNWANTED &&
              pickResult < Pickit.Result.TRASH:
  ...
  getCorpse() {
    ...
    if (!me.getItem(-1, sdk.items.mode.Equipped)) {
      corpse = Misc.poll(
        () => Game.getPlayer(me.name, sdk.player.mode.Dead),
        2500,
        500
      );
    } else {
  ...
  clearScrolls() {
    let scrolls = me
      ...
      .filter(
        (scroll) =>
          scroll.isInInventory && scroll.itemType === sdk.items.type.Scroll
      );
  ...
  clearInventory() {
    ...
    Config.DebugMode &&
      potsInInventory.length > 0 &&
      console.debug("clearInventory: start pots clean-up");
    ...
    potsInInventory.forEach(function (p) {
      ...
      for (let i = 0; i < 4 && !moved; i++) {
        ...
        if (
          freeSpace[i] > 0 &&
          p.code &&
          p.code.startsWith(Config.BeltColumn[i])
        ) {
          ...
          if (
            freeSpace[i] === beltSize ||
            freeSpace.some((spot) => spot === beltSize)
          ) {
            let x =
              freeSpace[i] === beltSize ? i : beltCapRef[i] - freeSpace[i] * 4;
            Packet.placeInBelt(p, x);
          } else {
            Game.clickItemAndWait(
              sdk.clicktypes.click.ShiftLeft,
              p.x,
              p.y,
              p.location
            );
    ...
    if (items.length > 0) {
      ...
      if (sell.length) {
        Config.DebugMode &&
          console.debug(
            "PreSellLen: " + sell.length + " PreDropLen: " + drop.length
          );
        ...
        Config.DebugMode &&
          console.debug(
            "AfterSellLen: " + sell.length + " AfterDropLen: " + drop.length
          );
  ...
  initialize() {
    ...
    switch (me.act) {
      case 1:
        let wp = Game.getPresetObject(
          sdk.areas.RogueEncampment,
          sdk.objects.A1Waypoint
        );
        let fireUnit = Game.getPresetObject(
          sdk.areas.RogueEncampment,
          sdk.objects.A1TownFire
        );
  ...
  move(spot = "", allowTK = true) {
    ...
    if (me.act === 5) {
      ...
      if (spot === "portalspot" && getDistance(me.x, me.y, 5113, 5068) <= 8) {
        path = [
          5113, 5068, 5108, 5051, 5106, 5046, 5104, 5041, 5102, 5027, 5098,
          5018,
        ];
        returnWhenDone = true;
      }
  ...
  moveToSpot(spot = "", allowTK = true) {
    let townSpot;
    let longRange = !Skill.haveTK && spot === "waypoint";
    let tkRange =
      Skill.haveTK &&
      allowTK &&
      ["stash", "portalspot", "waypoint"].includes(spot);
  ...
  //MTODO 2848 line die bug need fixed
  goToTown(act = 0, wpmenu = false) {
    if (!me.inTown) {
      try {
        ...
        if (!tpTool && Misc.getPlayerCount() <= 1) {
          Misc.errorReport(
            new Error(
              "Town.goToTown: Failed to go to town and no tps available. Restart."
            )
          );
          ...
          if (
            ...
            Misc.errorReport(
              new Error("Town.goToTown: Failed to go to town. Quiting.")
            );
            scriptBroadcast("quit");
          }
        }
