...
export const ClassAttack = {
  doAttack(unit, preattack = false) {
    ...
    if (!Attack.checkResist(unit, attackSkill)) {
      attackSkill = -1;
      //iomars
      let imIndex = unit.isSpecial ? 5 : 6;
      if (
        Config.AttackSkill[imIndex] > -1 &&
        Attack.checkResist(unit, Config.AttackSkill[imIndex])
      ) {
        attackSkill = Config.AttackSkill[imIndex];
      }
    }
    ...
    if (
      attackSkill === sdk.skills.DoubleThrow &&
      (me.getWeaponQuantity() <= 3 ||
        me.getWeaponQuantity(sdk.body.LeftArm) <= 3) &&
    ...
    if (monster) {
      do {
        if (
          ...
          (Attack.checkResist(
            monster,
            Attack.getSkillElement(
              Config.AttackSkill[monster.isSpecial ? 1 : 3]
            )
          ) ||
    ...
    MainLoop: for (let i = 0; i < 3; i += 1) {
      ...
      if (corpse) {
        do {
          if (
            corpse.dead &&
            getDistance(corpse, orgX, orgY) <= range &&
