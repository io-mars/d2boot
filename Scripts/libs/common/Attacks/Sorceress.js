...
export const ClassAttack = {
  ...
  doAttack(unit, preattack = false) {
      ...
      if (unit) {
        switch (true) {
          ...
          case typeof id === "string" &&
            unit.name &&
            unit.name.toLowerCase() === id.toLowerCase():
    ...
    if (
      useStatic &&
      Config.StaticList.some((id) => idCheck(id)) &&
      ...
      while (
        ...
        if (
          unit.distance > staticRange ||
          checkCollision(me, unit, sdk.collision.Ranged)
        ) {
          if (
            !Attack.getIntoPosition(unit, staticRange, sdk.collision.Ranged)
          ) {
            return Attack.Result.FAILED;
          }
        }
    ...
    if (result === Attack.Result.CANTATTACK && Attack.canTeleStomp(unit)) {
      ...
      while (unit.attackable) {
        ...
        if (!!closeMob) {
          ...
          this.doCast(closeMob, findSkill.timed, findSkill.untimed) === 1 ||
            (Skill.haveTK &&
  ...
  doCast(unit, timedSkill = -1, untimedSkill = -1) {
    ...
    if (
      timedSkill > -1 &&
      (!me.skillDelay || !Skill.isTimed(timedSkill)) &&
