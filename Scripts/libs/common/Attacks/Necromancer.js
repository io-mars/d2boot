...
import { Skill, Game, Misc } from "../Misc.js";
...
export const ClassAttack = {
  ...
  isArmyFull() {
    ...
    if (
      me.getMinionCount(sdk.summons.type.Skeleton) < this.maxSkeletons &&
  ...
  canCurse(unit, curseID) {
    ...
    let state = (() => {
      switch (curseID) {
        ...
        default:
          console.warn(
            "(\xFFc9canCurse) :: \xFFc1Invalid Curse ID: " + curseID
          );
  ...
  getCustomCurse(unit) {
    ...
    let curse = Config.CustomCurse.findIndex(function (unitID) {
      if (
        (typeof unitID[0] === "number" &&
          unit.classid &&
          unit.classid === unitID[0]) ||
        (typeof unitID[0] === "string" &&
          unit.name &&
          unit.name.toLowerCase() === unitID[0].toLowerCase())
      ) {
        return true;
      }
      return false;
    ...
    if (curse > -1) {
      ...
      if (Config.CustomCurse[curse].length === 3) {
        return unit.spectype & Config.CustomCurse[curse][2]
          ? Config.CustomCurse[curse][1]
          : false;
      } else {
  ...
  doAttack(unit, preattack = false) {
    ...
    if (
      ...
      if (
        unit.distance > Skill.getRange(Config.AttackSkill[0]) ||
        checkCollision(me, unit, sdk.collision.Ranged)
      ) {
        if (
          !Attack.getIntoPosition(
            unit,
            Skill.getRange(Config.AttackSkill[0]),
      ...
      Skill.cast(
        Config.AttackSkill[0],
        Skill.getHand(Config.AttackSkill[0]),
        unit
      );
    ...
    if (unit.curseable) {
      ...
      if (customCurse && this.canCurse(unit, customCurse)) {
        ...
        if (
          Config.Curse[0] > 0 &&
          unit.isSpecial &&
          this.canCurse(unit, Config.Curse[0])
        ) {
          if (
            unit.distance > 25 ||
            checkCollision(me, unit, sdk.collision.Ranged)
        ...
        if (
          Config.Curse[1] > 0 &&
          !unit.isSpecial &&
          this.canCurse(unit, Config.Curse[1])
        ) {
          if (
            unit.distance > 25 ||
            checkCollision(me, unit, sdk.collision.Ranged)
    ...
    if (
      Attack.checkResist(unit, checkSkill) &&
    ...
    if (result === 1) {
      ...
      result === Attack.Result.CANTATTACK &&
      Attack.canTeleStomp(unit)
    ) {
      let merc = me.getMerc();
  ...
  doCast(unit, timedSkill = -1, untimedSkill = -1) {
    ...
    this.isArmyFull() &&
      this.checkCorpseNearMonster(unit) &&
      this.explodeCorpses(unit);
    ...
    if (timedSkill > -1 && (!me.skillDelay || !Skill.isTimed(timedSkill))) {
      switch (timedSkill) {
        case sdk.skills.PoisonNova:
          if (
            !this.novaTick ||
            getTickCount() - this.novaTick > Config.PoisonNovaDelay * 1000
          ) {
            if (
              unit.distance > Skill.getRange(timedSkill) ||
              checkCollision(me, unit, sdk.collision.Ranged)
            ) {
              if (
                !Attack.getIntoPosition(
        ...
        default:
          if (
            Skill.getRange(timedSkill) < 4 &&
            !Attack.validSpot(unit.x, unit.y, timedSkill, classid)
          )
            return Attack.Result.FAILED;
    ...
    if (untimedSkill > -1) {
      if (
        Skill.getRange(untimedSkill) < 4 &&
        !Attack.validSpot(unit.x, unit.y, untimedSkill, classid)
      )
        return Attack.Result.FAILED;

      if (
        unit.distance > Skill.getRange(untimedSkill) ||
        checkCollision(me, unit, sdk.collision.Ranged)
      ) {
        // Allow short-distance walking for melee skills
        walk =
    ...
    while (
      timedSkill === sdk.skills.PoisonNova &&
      this.novaTick &&
      getTickCount() - this.novaTick < Config.PoisonNovaDelay * 1000
    ) {
      delay(40);
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      while (corpseList.length > 0) {
        ...
        if (me.getMinionCount(sdk.summons.type.Skeleton) < this.maxSkeletons) {
          if (
            !Skill.cast(sdk.skills.RaiseSkeleton, sdk.skills.hand.Right, corpse)
          ) {
            return false;
          }
  ...
  explodeCorpses(unit) {
    ...
    let range = Math.floor(
      (me.getSkill(Config.ExplodeCorpses, sdk.skills.subindex.SoftPoints) + 7) /
        3
    );
    let corpse = Game.getMonster(-1, sdk.monsters.mode.Dead);
      ...
      if (this.isArmyFull()) {
        ...
        for (let i = 0; i <= 1; i += 1) {
          if (corpseList.length > 0) {
            ...
            if (corpse) {
              me.overhead(
                `\xFFc1Exploding:\xFFc0 ${corpse.name} (${corpse.classid})`
              );
  ...
  checkCorpseNearMonster(monster, range) {
    let corpse = Game.getMonster(-1, sdk.monsters.mode.Dead);

    // Assume CorpseExplosion if no range specified
    range === undefined &&
      (range = Math.floor(
        (me.getSkill(Config.ExplodeCorpses, sdk.skills.subindex.SoftPoints) +
          7) /
          3
      ));
  ...
  checkCorpse(unit, revive = false) {
    ...
    if (
      revive &&
      (unit.isSpecial ||
        badList.includes(baseId) ||
        (Config.ReviveUnstackable &&
          getBaseStat("monstats2", baseId, "sizex") === 3))
    ) {
      return false;
    }
