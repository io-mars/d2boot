...
export const ClassAttack = {
    ...
    if (result === Attack.Result.CANTATTACK && Attack.canTeleStomp(unit)) {
      ...
      while (unit.attackable) {
        ...
        if (!!closeMob) {
          ...
          this.doCast(closeMob, findSkill.timed, findSkill.untimed) ===
            Attack.Result.SUCCESS ||
            (Skill.canUse(sdk.skills.Decoy) &&
              Skill.cast(sdk.skills.Decoy, sdk.skills.hand.Right, unit));
        }
      }
  ...
  doCast(unit, timedSkill = -1, untimedSkill = -1) {
    ...
    if (this.bowCheck) {
      switch (true) {
        case this.bowCheck === "bow" &&
          !me.getItem("aqv", sdk.items.mode.Equipped):
        case this.bowCheck === "crossbow" &&
    ...
    if (timedSkill > -1 && (!me.skillDelay || !Skill.isTimed(timedSkill))) {
      switch (timedSkill) {
        case sdk.skills.LightningFury:
          if (
            !this.lightFuryTick ||
            getTickCount() - this.lightFuryTick >
              Config.LightningFuryDelay * 1000
          ) {
            if (
        ...
        default:
          if (
            ...
            !Attack.validSpot(unit.x, unit.y, timedSkill, unit.classid)
          ) {
            return Attack.Result.FAILED;
          }
    ...
    if (untimedSkill > -1) {
      if (
        ...
        !Attack.validSpot(unit.x, unit.y, untimedSkill, unit.classid)
      ) {
        return Attack.Result.FAILED;
      }
    ...
    while (
      timedSkill === sdk.skills.LightningFury &&
      this.lightFuryTick &&
      getTickCount() - this.lightFuryTick < Config.LightningFuryDelay * 1000
    ) {
      delay(40);
    }
