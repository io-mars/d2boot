...
export const ClassAttack = {
  ...
  doAttack(unit, preattack) {
    ...
    if (!this.feralBoost && Config.AttackSkill.includes(sdk.skills.FeralRage)) {
      ...
      this.feralBoost =
        (Math.floor(
          me.getSkill(sdk.skills.FeralRage, sdk.skills.subindex.SoftPoints) / 2
        ) +
          3) *
          4 +
        this.baseLL;
    ...
    if (!this.maulBoost && Config.AttackSkill.includes(sdk.skills.Maul)) {
      ...
      this.maulBoost =
        (Math.floor(
          me.getSkill(sdk.skills.Maul, sdk.skills.subindex.SoftPoints) / 2
        ) +
          3) *
          20 +
        this.baseED;
    ...
    if (
      ((Config.AttackSkill[0] === sdk.skills.FeralRage &&
        (!me.getState(sdk.states.FeralRage) ||
          me.getStat(sdk.stats.LifeLeech) < this.feralBoost)) ||
        (Config.AttackSkill[0] === sdk.skills.Maul &&
          (!me.getState(sdk.states.Maul) ||
            me.getStat(sdk.stats.DamagePercent) < this.maulBoost)) ||
        (Config.AttackSkill[0] === sdk.skills.ShockWave &&
          !unit.isSpecial &&
          !unit.getState(sdk.states.Stunned)) ||
    ...
    // Rebuff Armageddon
    Skill.canUse(sdk.skills.Armageddon) &&
      !me.getState(sdk.states.Armageddon) &&
    ...
    if (
      ...
      Skill.wereFormCheck(checkSkill) &&
    ...
    // eval skills
    switch (true) {
      case timedSkill === sdk.skills.Fury &&
        untimedSkill === sdk.skills.FeralRage:
        if (
          !me.getState(sdk.states.FeralRage) ||
          me.getStat(sdk.stats.LifeLeech) < this.feralBoost
        ) {
      ...
      case timedSkill === sdk.skills.FireClaws &&
        untimedSkill === sdk.skills.Rabies:
        if (!unit.getState(sdk.states.Rabies)) {
          timedSkill = sdk.skills.Rabies;
        }

        break;
      case timedSkill === sdk.skills.ShockWave &&
        untimedSkill === sdk.skills.Maul:
      case timedSkill === sdk.skills.Maul &&
        untimedSkill === sdk.skills.ShockWave:
      case timedSkill === sdk.skills.Maul &&
        untimedSkill === sdk.skills.FireClaws:
    ...
    let choosenSkill =
      Skill.isTimed(timedSkill) && me.skillDelay && untimedSkill > -1
        ? untimedSkill
        : timedSkill;
  ...
  doCast(unit, skill) {
    ...
    if (
      unit.distance > Skill.getRange(skill) ||
      checkCollision(me, unit, sdk.collision.WallOrRanged)
    ) {
      if (
        !Attack.getIntoPosition(
          unit,
          Skill.getRange(skill),
          sdk.collision.WallOrRanged,
          true
        )
      ) {
