...
export const ClassAttack = {
  attackAuras: [
    sdk.skills.HolyFire,
    sdk.skills.HolyFreeze,
    sdk.skills.HolyShock,
  ],
    ...
    if (this.attackAuras.includes(aura)) {
      ...
      if (!Attack.checkResist(unit, aura)) {
        ...
        if (
          ...
          Attack.checkResist(
            unit,
            this.attackAuras.includes(Config.AttackSkill[6])
  ...
  afterAttack() {
    ...
    if (
      ...
      Config.Redemption.some((v) => v > 0)
    ) {
      if (
        (me.hpPercent < Config.Redemption[0] ||
          me.mpPercent < Config.Redemption[1]) &&
        Attack.checkNearCorpses(me) > 2 &&
  ...
  doCast(unit, attackSkill = -1, aura = -1) {
    ...
    switch (attackSkill) {
      ...
      case sdk.skills.HolyBolt:
        if (
          unit.distance > Skill.getRange(attackSkill) + 3 ||
          CollMap.checkColl(me, unit, sdk.collision.Ranged)
        ) {
          if (
            !Attack.getIntoPosition(
              unit,
              Skill.getRange(attackSkill),
              sdk.collision.Ranged
            )
          ) {
            return Attack.Result.FAILED;
        ...
        if (
          unit.distance > Skill.getRange(attackSkill) ||
      ...
      case sdk.skills.Vengeance:
        ...
        if (
          ...
          if (
            !Attack.getIntoPosition(unit, 3, sdk.collision.WallOrRanged, true)
          ) {
            return Attack.Result.FAILED;
          }
        }
      ...
      default:
        if (
          ...
          !Attack.validSpot(unit.x, unit.y, attackSkill, unit.classid)
        )
          return Attack.Result.FAILED;
  ...
  dollAvoid(unit) {
    ...
    for (let i = 0; i < 2 * Math.PI; i += Math.PI / 6) {
      ...
      if (Attack.validSpot(unit.x + cx, unit.y + cy)) {
        ...
        return Pather.moveToEx(unit.x + cx, unit.y + cy, {
          clearSettings: { allowClearing: false },
        });
      }
    }
  ...
  getHammerPosition(unit) {
    ...
    switch (unit.type) {
      ...
      case sdk.unittype.Monster:
        ...
        x =
          commonCheck && getDistance(me, unit.targetx, unit.targety) > 5
            ? unit.targetx
            : unit.x;
        y =
          commonCheck && getDistance(me, unit.targetx, unit.targety) > 5
    ...
    for (let i = 0; i < positions.length; i += 1) {
      ...
      if (
        Attack.validSpot(check.x, check.y) &&
        !CollMap.checkColl(unit, check, sdk.collision.BlockWalk, 0)
      ) {
    ...
    console.debug(
      "Failed to find a hammer position for " +
        unit.name +
        " distance from me: " +
        unit.distance
    );
    ...
    if ([x, y].distance > 0) {
      if (Pather.useTeleport()) {
        ...
        if ([x, y].distance <= 4) {
          ...
          !CollMap.checkColl(me, { x: x, y: y }, sdk.collision.BlockWalk, 3)
        ) {
          Pather.walkTo(x, y);
        } else {
          // don't clear while trying to reposition
