...
export const Runeword = {
  ...
  Death: me.ladder
    ...
    : false, // Hel + El + Vex + Ort + Gul
  Destruction: me.ladder
    ? [
        sdk.items.runes.Vex,
        sdk.items.runes.Lo,
        sdk.items.runes.Ber,
    ...
    : false, // Vex + Lo + Ber + Jah + Ko
  Dragon: me.ladder
    ? [sdk.items.runes.Sur, sdk.items.runes.Lo, sdk.items.runes.Sol]
    : false, // Sur + Lo + Sol
  Dream: me.ladder
    ? [sdk.items.runes.Io, sdk.items.runes.Jah, sdk.items.runes.Pul]
    : false, // Io + Jah + Pul
  Edge: me.ladder
    ? [sdk.items.runes.Tir, sdk.items.runes.Tal, sdk.items.runes.Amn]
    : false, // Tir + Tal + Amn
  Faith: me.ladder
    ? [
        sdk.items.runes.Ohm,
  ...
  LastWish: me.ladder
    ...
    : false, // Jah + Mal + Jah + Sur + Jah + Ber
  Lawbringer: me.ladder
    ? [sdk.items.runes.Amn, sdk.items.runes.Lem, sdk.items.runes.Ko]
    : false, // Amn + Lem + Ko
  Oath: me.ladder
    ? [
        sdk.items.runes.Shael,
  ...
  Spirit: me.ladder
    ...
    : false, // Tal + Thul + Ort + Amn
  VoiceofReason: me.ladder
    ? [
        sdk.items.runes.Lem,
        sdk.items.runes.Ko,
        sdk.items.runes.El,
...
export const Runewords = {
  ...
  init() {
    ...
    for (let i = 0; i < Config.KeepRunewords.length; i += 1) {
      ...
      parsedLine &&
        this.pickitEntries.push(
          NTIP.ParseLineInt(Config.KeepRunewords[i], info)
        );
    }
        ...
        if (isNaN(Config.Runewords[i][1])) {
          if (
            NTIPAliasClassID.hasOwnProperty(
              Config.Runewords[i][1].replace(/\s+/g, "").toLowerCase()
            )
          ) {
            ...
            Misc.errorReport(
              "\xFFc1Invalid runewords entry:\xFFc0 " + Config.Runewords[i][1]
            );
  ...
  buildLists() {
    ...
    for (let i = 0; i < Config.Runewords.length; i += 1) {
      ...
      if (
        ...
        RuneLoop: for (let j = 0; j < Config.Runewords[i][0].length; j += 1) {
          for (let k = 0; k < items.length; k += 1) {
            if (
              items[k].classid === Config.Runewords[i][0][j] &&
              this.validItem(items[k])
            ) {
    ...
    for (let i = 0; i < Config.Runewords.length; i += 1) {
      ...
      if (base) {
        ...
        for (let j = 0; j < Config.Runewords[i][0].length; j += 1) {
          ...
          if (itemList.length === Config.Runewords[i][0].length + 1) {
            // runes + base
  ...
  checkItem(unit) {
    ...
    return (
      ...
      this.needList.includes(unit.classid)
    );
  },
  ...
  /* get the base item based on classid and runeword recipe
	...
	*/
  getBase(runeword, base, ethFlag, reroll) {
    let item =
      typeof base === "object"
        ? base
      ...
      do {
        if (
          ...
          if (
            ...
            if (
              !ethFlag ||
              (ethFlag === Roll.Eth && item.ethereal) ||
              (ethFlag === Roll.NonEth && !item.ethereal)
            ) {
  ...
  getScroll() {
    let scroll = me.getItem(
      sdk.items.ScrollofTownPortal,
      sdk.items.mode.inStorage
    ); // check if we already have the scroll
  ...
  makeRunewords() {
    ...
    while (true) {
      ...
      print(
        "\xFFc4Runewords: \xFFc0Made runeword: " +
          items[0].fname.split("\n").reverse().join(" ").omitColor()
      );
  ...
  rerollRunewords() {
    for (let i = 0; i < Config.Runewords.length; i += 1) {
      ...
      if (base) {
        ...
        print(
          "\xFFc4Runewords: \xFFc0Rerolling runeword: " +
            base.fname.split("\n").reverse().join(" ").omitColor()
        );
        D2Bot.printToConsole(
          "Rerolling runeword: " +
