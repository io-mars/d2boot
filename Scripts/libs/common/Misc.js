...
export const Skill = {
  ...
  skills: {
    initialized: false,
    all: [],
    addSkills(skill, condition = () => true) {
      this.all[skill] = {
        hardpoints: false,
        checked: false,
        condition: condition,
        have() {
          ...
          if (!this.checked) {
            this.hardpoints = !!me.getSkill(
              skill,
              sdk.skills.subindex.HardPoints
            );
            this.checked = true;
          }
          return (
            this.hardpoints ||
            me.getSkill(skill, sdk.skills.subindex.SoftPoints)
          );
        },
      };
    },
    init() {
      ...
      this.addSkills(
        sdk.skills.BurstofSpeed,
        () => (Config.UseBoS && me.inTown) || (!Config.UseFade && !me.inTown)
        // () => !Config.UseBoS && !me.inTown
      );
    ...
    switch (me.classid) {
      ...
      case sdk.player.class.Sorceress:
        if (Config.UseColdArmor === true) {
          Precast.skills.coldArmor.best = (function () {
            let coldArmor = [
              {
                skillId: sdk.skills.ShiverArmor,
                level: me.getSkill(
              ...
              {
                skillId: sdk.skills.ChillingArmor,
                level: me.getSkill(
                  sdk.skills.ChillingArmor,
                  sdk.skills.subindex.SoftPoints
                ),
              },
              {
                skillId: sdk.skills.FrozenArmor,
                level: me.getSkill(
          ...
          Precast.skills.coldArmor.duration = this.getDuration(
            Precast.skills.coldArmor.best
          );
        } else {
          Precast.skills.coldArmor.duration = this.getDuration(
            Config.UseColdArmor
      ...
      case sdk.player.class.Barbarian:
        Skill.canUse(sdk.skills.Shout) &&
          (Precast.skills.shout.duration = this.getDuration(sdk.skills.Shout));
        Skill.canUse(sdk.skills.BattleOrders) &&
          (Precast.skills.battleOrders.duration = this.getDuration(
            sdk.skills.BattleOrders
          ));
        Skill.canUse(sdk.skills.BattleCommand) &&
          (Precast.skills.battleCommand.duration = this.getDuration(
  ...
  canUse(skillId = -1) {
    try {
      ...
      if (skillId >= sdk.skills.Attack && skillId <= sdk.skills.LeftHandSwing)
        return true;
  ...
  getDuration(skillId = -1) {
    switch (skillId) {
      case sdk.skills.Decoy:
        return (
          (10 +
            me.getSkill(sdk.skills.Decoy, sdk.skills.subindex.SoftPoints) * 5) *
          1000
        );
      ...
      case sdk.skills.Shout:
        return (
          (10 +
            me.getSkill(sdk.skills.Shout, sdk.skills.subindex.SoftPoints) * 10 +
            (me.getSkill(
              sdk.skills.BattleOrders,
              sdk.skills.subindex.HardPoints
            ) +
      ...
      case sdk.skills.HolyShield:
        return (
          5 +
          25 *
            me.getSkill(sdk.skills.HolyShield, sdk.skills.subindex.SoftPoints) *
            1000
        );
      ...
      case sdk.skills.Fade:
        return (
          108 +
          12 *
            me.getSkill(sdk.skills.Fade, sdk.skills.subindex.SoftPoints) *
            1000
        );
      case sdk.skills.Venom:
        return (
          116 +
          4 *
            me.getSkill(sdk.skills.Venom, sdk.skills.subindex.SoftPoints) *
            1000
        );
  ...
  getMaxSummonCount(skillId) {
    let skillNum = 0;

    switch (skillId) {
      case sdk.skills.Raven:
        return Math.min(
          me.getSkill(skillId, sdk.skills.subindex.SoftPoints),
          5
        );
  ...
  getRange(skillId) {
    switch (skillId) {
      ...
      case sdk.skills.Leap: {
        let skLvl = me.getSkill(
          sdk.skills.Leap,
          sdk.skills.subindex.SoftPoints
        );
        return Math.floor(
          Math.min(4 + 26 * ((110 * skLvl) / (skLvl + 6) / 100), 30) * (2 / 3)
        );
      }
      case sdk.skills.ArcticBlast: {
        ...
        range > 6 &&
          Config.AttackSkill[5] === sdk.skills.ArcticBlast &&
          (range = 6);
  ...
  missileSkills: [
    sdk.skills.MagicArrow,
    sdk.skills.FireArrow,
    sdk.skills.ColdArrow,
    sdk.skills.MultipleShot,
    sdk.skills.PoisonJavelin,
    sdk.skills.ExplodingArrow,
    sdk.skills.LightningBolt,
    sdk.skills.IceArrow,
    sdk.skills.GuidedArrow,
    sdk.skills.PlagueJavelin,
    sdk.skills.Strafe,
    sdk.skills.ImmolationArrow,
    sdk.skills.FreezingArrow,
    ...
    sdk.skills.Teeth,
    sdk.skills.BoneSpear,
    sdk.skills.BoneSpirit,
    sdk.skills.HolyBolt,
    sdk.skills.FistoftheHeavens,
    sdk.skills.DoubleThrow,
    sdk.skills.Firestorm,
    sdk.skills.MoltenBoulder,
    sdk.skills.ArcticBlast,
    sdk.skills.Twister,
    sdk.skills.Tornado,
  ...
  cast(skillId, hand, x, y, item) {
    switch (true) {
      case me.inTown && !this.townSkill(skillId):
      case !item &&
        (this.getManaCost(skillId) > me.mp || !this.canUse(skillId)):
    ...
    if (
      skillId === sdk.skills.Telekinesis &&
      typeof x === "object" &&
      Packet.telekinesis(x)
    ) {
      delay(250);
      return true;
    }
    ...
    if (!item && this.getManaCost(skillId) > me.mp) {
      ...
      if (
        Config.AttackSkill.concat([sdk.skills.StaticField, sdk.skills.Teleport])
          .concat(Config.LowManaSkill)
          .includes(skillId)
      ) {
        delay(300);
      }
  ...
  setSkill(skillId, hand, item) {
    ...
    if (hand === undefined || hand === sdk.skills.hand.RightShift || item) {
      item &&
        hand !== sdk.skills.hand.Right &&
        console.warn(
          "[\xFFc9Warning\xFFc0] charged skills must be cast from right hand"
        );
      hand = sdk.skills.hand.Right;
    }
  ...
  isTimed(skillId) {
    return [
      ...
      sdk.skills.Volcano,
      sdk.skills.Grizzly,
      sdk.skills.Armageddon,
      sdk.skills.Hurricane,
      sdk.skills.ShockWeb,
      sdk.skills.ShadowWarrior,
      sdk.skills.DragonFlight,
      sdk.skills.BladeShield,
  ...
  wereFormCheck(skillId) {
    ...
    if (
      !Skill.canUse(sdk.skills.Werewolf) &&
      !Skill.canUse(sdk.skills.Werebear)
    )
      return true;
    ...
    if (
      [
        ...
        sdk.skills.CarrionVine,
        sdk.skills.HeartofWolverine,
        sdk.skills.SummonDireWolf,
        sdk.skills.FireClaws,
        sdk.skills.SolarCreeper,
        sdk.skills.Hunger,
    ...
    if (
      me.getState(sdk.states.Wearwolf) &&
      [
        sdk.skills.Werewolf,
    ...
    if (
      ...
      [sdk.skills.Werebear, sdk.skills.Maul, sdk.skills.ShockWave].includes(
        skillId
      )
    )
      return true;
  ...
  getManaCost(skillId) {
    ...
    let lvlmana =
      getBaseStat("skills", skillId, "lvlmana") === 65535
        ? -1
        : getBaseStat("skills", skillId, "lvlmana"); // Correction for skills that need less mana with levels (kolton)
    let ret = Math.max(
      (getBaseStat("skills", skillId, "mana") + lvlmana * (skillLvl - 1)) *
        (effectiveShift[getBaseStat("skills", skillId, "manashift")] / 256),
      getBaseStat("skills", skillId, "minmana")
    );
  ...
  useTK(unit = undefined) {
    try {
      if (
        !unit ||
        !Skill.canUse(sdk.skills.Telekinesis) ||
        typeof unit !== "object" ||
        unit.type !== sdk.unittype.Object ||
        unit.name.toLowerCase() === "dummy" ||
        (unit.name.toLowerCase() === "portal" &&
          !me.inTown &&
          unit.classid !== sdk.objects.ArcaneSanctuaryPortal) ||
        [
          sdk.objects.RedPortalToAct4,
          sdk.objects.WorldstonePortal,
...
export const Misc = {
  ...
  // Get player within getUnit distance
  getNearbyPlayer() {
  ...
  getPartyCount() {
    ...
    if (party) {
      ...
      do {
        if (
          party.partyid !== sdk.party.NoParty &&
  ...
  checkPartyLevel(levelCheck = 1, exclude = []) {
  ...
  autoLeaderDetect(givenSettings = {}) {
    ...
    do {
      ...
      do {
        ...
        if (
          suspect.area === settings.destination &&
          !getPlayerFlag(me.gid, suspect.gid, 8)
        ) {
          leader = suspect.name; // ... is our leader
          console.log("\xFFc4Autodetected " + leader);
  ...
  openChest(unit) {
    ...
    if (!unit || unit.x === 12526 || unit.x === 12565 || unit.mode)
      return false;
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (useTK) {
        unit.distance > 13 &&
          Attack.getIntoPosition(unit, 13, sdk.collision.WallOrRanged);
        if (!Skill.cast(sdk.skills.Telekinesis, sdk.skills.hand.Right, unit)) {
          console.debug("Failed to tk: attempt: " + i);
          continue;
        }
      } else {
        [unit.x + 1, unit.y + 2].distance > 5 &&
          Pather.moveTo(unit.x + 1, unit.y + 2, 3);
        specialChest || i > 2
          ? Misc.click(0, 0, unit)
  ...
  openChestsInArea(area, chestIds = []) {
    !area && (area = me.area);
    ...
    if (!chestIds.length) {
      chestIds = [
        5, 6, 87, 104, 105, 106, 107, 143, 140, 141, 144, 146, 147, 148, 176,
        177, 181, 183, 198, 240, 241, 242, 243, 329, 330, 331, 332, 333, 334,
        335, 336, 354, 355, 356, 371, 387, 389, 390, 391, 397, 405, 406, 407,
        413, 420, 424, 425, 430, 431, 432, 433, 454, 455, 501, 502, 504, 505,
        580, 581,
      ];
    }
    ...
    while (coords.length) {
      ...
      for (let i = 0; i < coords.length; i += 1) {
        if (
          getDistance(coords[i].x, coords[i].y, coords[0].x, coords[0].y) < 20
        ) {
          coords.shift();
        }
      }
      ...
      for (let i = 0; i < Config.ScanShrines.length; i += 1) {
        for (let j = 0; j < shrineList.length; j += 1) {
          ...
          if (index === -1 || i <= index || this.shrineStates[i] === 0) {
            if (
              shrineList[j].objtype === Config.ScanShrines[i] &&
              (Pather.useTeleport() ||
                !checkCollision(me, shrineList[j], sdk.collision.WallOrRanged))
            ) {
  ...
  getShrinesInArea(area, type, use) {
    let shrineLocs = [];
    ...
    try {
      ...
      while (shrineLocs.length > 0) {
        ...
        Skill.haveTK
          ? Pather.moveNear(coords[0], coords[1], 20)
  ...
  getItemDesc(unit, logILvl = true) {
    ...
    for (let i = 0; i < desc.length; i += 1) {
      ...
      if (desc[i].includes(getLocaleString(sdk.locale.text.SellValue))) {
        ...
        let index = desc[i].lastIndexOf("\xFFc");
    ...
    if (logILvl && desc[desc.length - 1]) {
      desc[desc.length - 1] =
        desc[desc.length - 1].trim() + " (" + unit.ilvl + ")";
    }
  ...
  getItemCode(unit) {
      ...
      switch (unit.quality) {
        ...
        case sdk.items.quality.Unique:
          for (let i = 0; i < 401; i += 1) {
            if (
              unit.code === getBaseStat("uniqueitems", i, 4).trim() &&
              unit.fname
                .split("\n")
                .reverse()[0]
                .includes(getLocaleString(getBaseStat("uniqueitems", i, 2)))
            ) {
    ...
    if (!code) {
      ...
      [
        sdk.items.type.Ring,
        sdk.items.type.Amulet,
        sdk.items.type.Jewel,
  ...
  itemLogger(action, unit, text) {
    ...
    return this.fileAction(
      ...
      dateString +
        ...
        action +
        "> (" +
        Pickit.itemQualityToName(unit.quality) +
        ") " +
        desc +
        (text ? " {" + text + "}" : "") +
        "\n"
  ...
  logItem(action, unit, keptLine) {
    if (!this.useItemLog) return false;
    if (!Config.LogKeys && ["pk1", "pk2", "pk3"].includes(unit.code))
      return false;
    if (!Config.LogOrgans && ["dhn", "bey", "mbr"].includes(unit.code))
      return false;
    if (
      !Config.LogLowRunes &&
      [
      ...
      return false;
    if (
      !Config.LogMiddleRunes &&
      ["r15", "r16", "r17", "r18", "r19", "r20", "r21", "r22", "r23"].includes(
        unit.code
      )
    )
      return false;
    ...
    for (let i = 0; i < Config.SkipLogging.length; i++) {
      if (
        Config.SkipLogging[i] === unit.classid ||
        Config.SkipLogging[i] === unit.code
      )
        return false;
    }
  ...
  skipItem(id) {
    return [
      ...
      sdk.items.Wand,
      sdk.items.Club,
      sdk.items.ShortSword,
      sdk.items.Javelin,
      sdk.items.ShortStaff,
      sdk.items.Katar,
      sdk.items.Buckler,
      sdk.items.StaminaPotion,
      sdk.items.AntidotePotion,
      sdk.items.RejuvenationPotion,
      ...
      sdk.items.Key,
      sdk.items.MinorHealingPotion,
      sdk.items.LightHealingPotion,
      sdk.items.HealingPotion,
      sdk.items.GreaterHealingPotion,
      sdk.items.SuperHealingPotion,
      sdk.items.MinorManaPotion,
      sdk.items.LightManaPotion,
      sdk.items.ManaPotion,
      sdk.items.GreaterManaPotion,
  ...
  townCheck() {
    ...
    if (Config.TownCheck && !me.inTown) {
      try {
        if (
          Town.needPotions() ||
          (Config.OpenChests.Enabled && Town.needKeys())
        ) {
          check = true;
        }
      } catch (e) {
  ...
  fileAction(path, mode, msg) {
    let contents = "";
    MainLoop: for (let i = 0; i < 30; i += 1) {
      try {
        switch (mode) {
          case this.FileActionMode.read: // read
            contents = FileTools.readText(path);

            break MainLoop;
          case this.FileActionMode.write: // write
            FileTools.writeText(path, msg);

            break MainLoop;
          case this.FileActionMode.append: // append
  ...
  errorReport(error, script) {
    ...
    if (typeof error === "string") {
      msg = error;
      oogmsg = error.omitColor();
      filemsg =
        dateString + " <" + me.profile + "> " + error.omitColor() + "\n";
    } else {
      //MTODO stack??
      source = error.stack.substring(
        error.stack.lastIndexOf("\\") + 1,
        error.stack.length
      );
      msg =
        "\xFFc1Error in \xFFc0" +
      ...
      oogmsg =
        ...
        me.gamename +
        ")";
      filemsg = dateString + " <" + me.profile + "> " + msg.omitColor() + "\n";
      ...
      if (error.hasOwnProperty("stack")) {
        ...
        if (stack) {
          ...
          for (let i = 0; i < stack.length; i += 1) {
            if (stack[i]) {
              stackLog +=
                stack[i].substr(0, stack[i].indexOf("@") + 1) +
                stack[i].substr(
  ...
  poll(check, timeout = 6000, sleep = 40) {
    let ret,
      start = getTickCount();
...
export const Sort = {
  ...
  units: function (a, b) {
    return (
      Math.round(getDistance(me.x, me.y, a.x, a.y)) -
      Math.round(getDistance(me.x, me.y, b.x, b.y))
    );
  },
  ...
  presetUnits: function (a, b) {
    return (
      getDistance(me, a.roomx * 5 + a.x, a.roomy * 5 + a.y) -
...
export const Experience = {
  totalExp: [
    0, 0, 500, 1500, 3750, 7875, 14175, 22680, 32886, 44396, 57715, 72144,
    ...
    3902260, 4663553, 5493363, 6397855, 7383752, 8458379, 9629723, 10906488,
    ...
    624419793, 681027665, 742730244, 809986056, 883294891, 963201521,
    1050299747, 1145236814, 1248718217, 1361512946, 1484459201, 1618470619,
    1764543065, 1923762030, 2097310703, 2286478756, 2492671933, 2717422497,
    2962400612, 3229426756, 3520485254, 0, 0,
  ],
  nextExp: [
    0, 500, 1000, 2250, 4125, 6300, 8505, 10206, 11510, 13319, 14429, 18036,
    22545, 28181, 35226, 44033, 55042, 68801, 86002, 107503, 134378, 167973,
    209966, 262457, 328072, 410090, 512612, 640765, 698434, 761293, 829810,
    904492, 985897, 1074627, 1171344, 1276765, 1391674, 1516924, 1653448,
    1802257, 1964461, 2141263, 2333976, 2544034, 2772997, 3022566, 3294598,
    3591112, 3914311, 4266600, 4650593, 5069147, 5525370, 6022654, 6564692,
    7155515, 7799511, 8501467, 9266598, 10100593, 11009646, 12000515, 13080560,
    14257811, 15541015, 16939705, 18464279, 20126064, 21937409, 23911777,
    26063836, 28409582, 30966444, 33753424, 36791232, 40102443, 43711663,
    47645713, 51933826, 56607872, 61702579, 67255812, 73308835, 79906630,
    87098226, 94937067, 103481403, 112794729, 122946255, 134011418, 146072446,
    159218965, 173548673, 189168053, 206193177, 224750564, 244978115, 267026144,
    291058498, 0, 0,
  ],
  expCurve: [
    13, 16, 110, 159, 207, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
    255, 225, 174, 92, 38, 5,
  ],
  expPenalty: [
    1024, 976, 928, 880, 832, 784, 736, 688, 640, 592, 544, 496, 448, 400, 352,
    304, 256, 192, 144, 108, 81, 61, 46, 35, 26, 20, 15, 11, 8, 6, 5,
  ],
  monsterExp: [
    [1, 1, 1],
    [30, 78, 117],
  ...
  progress: function () {
    return me.getStat(sdk.stats.Level) === 99
      ? 0
      : (
          ((me.getStat(sdk.stats.Experience) -
            this.totalExp[me.getStat(sdk.stats.Level)]) /
            this.nextExp[me.getStat(sdk.stats.Level)]) *
          100
        ).toFixed(2);
  },
  ...
  timeToLevel: function () {
    let tTLrawSeconds = Math.floor(
      (getTickCount() - me.gamestarttime) / 1000
    ).toString();
    ...
    return (
      (tTLDays ? tTLDays + " d " : "") +
      (tTLHours ? tTLHours + " h " : "") +
      (tTLMinutes ? tTLMinutes + " m" : "")
    );
  },

  // Get Game Time
  getGameTime: function () {
    let rawMinutes = Math.floor(
      (getTickCount() - me.gamestarttime) / 60000
    ).toString();
    let rawSeconds = (
      Math.floor((getTickCount() - me.gamestarttime) / 1000) % 60
    ).toString();
  ...
  log: function () {
    ...
    let string =
      "[Game: " +
      me.gamename +
      (me.gamepassword ? "//" + me.gamepassword : "") +
      getGameTime +
      "] [Level: " +
      me.getStat(sdk.stats.Level) +
      " (" +
      progress +
      "%)] [XP: " +
      gain +
    ...
    if (gain) {
      ...
      if (me.getStat(sdk.stats.Level) > DataFile.getStats().level) {
        D2Bot.printToConsole(
          "Congrats! You gained a level. Current level:" +
            me.getStat(sdk.stats.Level),
          sdk.colors.D2Bot.Green
        );
      }
    }
  },
...
export const Game = {
  getDistance(...args) {
    switch (args.length) {
      ...
      case 1:
        ...
        return Math.sqrt(
          Math.pow(me.x - args[0].x, 2) + Math.pow(me.y - args[0].y, 2)
        );
      case 2:
        ...
        if (typeof args[0] === "number" && typeof args[1] === "number") {
          return Math.sqrt(
            Math.pow(me.x - args[0], 2) + Math.pow(me.y - args[1], 2)
          );
      ...
      case 3:
        ...
        if (!args[0].hasOwnProperty("x")) return Infinity;
        return Math.sqrt(
          Math.pow(args[0].x - args[1], 2) + Math.pow(args[0].y - args[2], 2)
        );
      case 4:
  ...
  clickItemAndWait(...args) {
    ...
    while (true) {
      // Wait until item is picked up.
      delay(3);

      if (
        before !== !!me.itemoncursor ||
        (timedOut =
          getTickCount() - timeout > Math.min(1000, 100 + me.ping * 4))
      ) {
...
export const Packet = {
    ...
    if (this.openMenu(unit)) {
      for (let i = 0; i < 10; i += 1) {
        ...
        if (unit.itemcount > 0) {
          ...
          console.info(
            false,
            "Successfully started " + mode + " at " + unit.name
          );
          return true;
        }
      }
  ...
  buyItem(unit, shiftBuy, gamble) {
    ...
    try {
      ...
      for (let i = 0; i < 3; i += 1) {
        sendPacket(
          1,
          sdk.packets.send.NPCBuy,
          4,
          npc.gid,
          4,
          unit.gid,
          4,
          shiftBuy ? 0x80000000 : gamble ? 0x2 : 0x0,
          4,
          0
        );
  ...
  buyScroll(unit, tome, shiftBuy) {
    let oldGold = me.gold;
    let itemCount = me.itemcount;
    let npc = getInteractedNPC();
    tome === undefined &&
      (tome = me.findItem(
        unit.classid === sdk.items.ScrollofTownPortal
          ? sdk.items.TomeofTownPortal
          : sdk.items.TomeofIdentify,
        sdk.items.mode.inStorage,
        sdk.storage.Inventory
  ...
  sellItem(unit) {
    ...
    if (unit.type !== sdk.unittype.Item)
  ...
  identifyItem(unit, tome) {
    if (!unit || unit.identified) return false;

    CursorLoop: for (let i = 0; i < 3; i += 1) {
    ...
    for (let i = 0; i < 3; i += 1) {
      getCursorType() === sdk.cursortype.Identify &&
        sendPacket(1, sdk.packets.send.IndentifyItem, 4, unit.gid, 4, tome.gid);
  ...
  telekinesis(who) {
    if (!who || !Skill.setSkill(sdk.skills.Telekinesis, sdk.skills.hand.Right))
      return false;
    sendPacket(
      1,
      sdk.packets.send.RightSkillOnEntityEx3,
  ...
  enchant(who) {
    if (!who || !Skill.setSkill(sdk.skills.Enchant, sdk.skills.hand.Right))
      return false;
    sendPacket(
      1,
      sdk.packets.send.RightSkillOnEntityEx3,
  ...
  teleport(wX, wY) {
    if (
      ![wX, wY].every((n) => typeof n === "number") ||
      !Skill.setSkill(sdk.skills.Teleport, sdk.skills.hand.Right)
    )
      return false;
  ...
  // moveNPC(npc, dwX, dwY) { // commented the patched packet
  // 	//sendPacket(1, sdk.packets.send.MakeEntityMove, 4, npc.type, 4, npc.gid, 4, dwX, 4, dwY);
  // },

  teleWalk(x, y, maxDist = 5) {
    !this.telewalkTick && (this.telewalkTick = 0);

    if (
      getDistance(me, x, y) > 10 &&
      getTickCount() - this.telewalkTick > 3000 &&
      Attack.validSpot(x, y)
    ) {
      for (let i = 0; i < 5; i += 1) {
        sendPacket(
          1,
          sdk.packets.send.UpdatePlayerPos,
          2,
          x + rand(-1, 1),
          2,
          y + rand(-1, 1)
        ...
        sendPacket(
          1,
          sdk.packets.send.RequestEntityUpdate,
          4,
          me.type,
          4,
          me.gid
  ...
  addListener(packetType, callback) {
    if (typeof packetType === "number") {
      packetType = [packetType];
    }

    if (typeof packetType === "object" && packetType.length) {
      addEventListener("gamepacket", (packet) =>
        packetType.indexOf(packet[0]) > -1 ? callback(packet) : false
      );
...
export const PacketBuilder = function () {
  ...
  this.buildDataView = () => {
    let dv = new DataView(new ArrayBuffer(count)),
      i = 0;
    queue.forEach((field) => {
...
export const LocalChat = function () {
  ...
  this.init = (cycle = false) => {
    ...
    print("\xFFc2LocalChat enabled. Mode: " + Config.LocalChat.Mode);
    ...
    if (Config.LocalChat.Toggle) {
      toggle =
        typeof Config.LocalChat.Toggle === "string"
...
export const Messaging = {
  ...
  sendToProfile: function (profileName, mode, message, getResponse = false) {
    ...
    function copyDataEvent({ mode: mode2, msg }) {
      if (mode2 === mode) {
        // let obj;
        // try {
        //   obj = JSON.parse(msg);
        ...
        if (msg.hasOwnProperty("sender") && msg.sender === profileName) {
          response = Misc.copy(msg);
        }
    ...
    let res = sendCopyData(
      null,
      profileName,
      mode,
      JSON.stringify({ message: message, sender: me.profile })
    );
