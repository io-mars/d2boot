...
import { Town, NPC } from "./Town.js";
import { Skill, Misc, Game, Packet, Time, Sort } from "./Misc.js";
...
export const NodeAction = {
  ...
  killMonsters(arg = {}) {
    ...
    if (
      ...
      let monList =
        Attack.getMob(sdk.monsters.Ghost1, sdk.monsters.spectype.All, 30) || [];
      monList.length > 0 && Attack.clearList(monList);
    }

    if (
      (typeof Config.ClearPath === "number" ||
        typeof Config.ClearPath === "object") &&
      ...
      switch (typeof Config.ClearPath) {
        ...
        case "object":
          if (
            !Config.ClearPath.hasOwnProperty("Areas") ||
...
const PathDebug = {
  ...
  drawPath(path) {
    ...
    for (let i = 0; i < path.length - 1; i += 1) {
      this.hooks.push(
        new Line(path[i].x, path[i].y, path[i + 1].x, path[i + 1].y, 0x84, true)
      );
    }
  },
...
export const Pather = {
  ...
  init() {
    if (!this.initialized) {
      me.classic &&
        (this.nonTownWpAreas = this.nonTownWpAreas.filter(
          (wp) => wp < sdk.areas.Harrogath
        ));
  ...
  canTeleport() {
    return (
      this.teleport &&
      (Skill.canUse(sdk.skills.Teleport) ||
        me.getStat(sdk.stats.OSkill, sdk.skills.Teleport))
    );
  },
  ...
  spotOnDistance(spot, distance, givenSettings = {}) {
    const settings = Object.assign(
      {},
      {
        area: me.area,
    ...
    if (!nodes.length) {
      ...
      if (!nodes.length)
        return settings.returnSpotOnError ? spot : { x: me.x, y: me.y };
    }
    ...
    return (
      nodes.find(
        (node) =>
          ...
          Pather.checkSpot(node.x, node.y, settings.coll)
      ) || (settings.returnSpotOnError ? spot : { x: me.x, y: me.y })
    );
  },

  move(target, givenSettings = {}) {
    // Abort if dead
    ...
    (!settings.allowClearing || !settings.clearSettings.clearPath) &&
      (settings.clearSettings.allowClearing = false);
    target instanceof PresetUnit &&
      (target = {
    ...
    if (settings.minDist > 3) {
      target = this.spotOnDistance(target, settings.minDist, {
        returnSpotOnError: settings.returnSpotOnError,
        reductionType: me.inTown ? 0 : 2,
      });
    }
    ...
    if (
      getDistance(me, target) < 2 &&
      !CollMap.checkColl(me, target, sdk.collision.BlockMissile, 5)
    )
      return true;

    let useTeleport = settings.allowTeleport && this.useTeleport();
    const tpMana = useTeleport
      ? Skill.getManaCost(sdk.skills.Teleport)
      : Infinity;
    const annoyingArea = [
    ...
    while (path.length > 0) {
      ...
      if (getDistance(me, node) > 2) {
        ...
        if (annoyingArea || invalidCheck) {
          ...
          annoyingArea &&
            ([
              settings.clearSettings.overrideConfig,
              settings.clearSettings.range,
            ] = [true, 5]);
          settings.retry <= 3 && !useTeleport && (settings.retry = 15);
        }

        if (
          useTeleport && tpMana <= me.mp
          ...
          if (!me.inTown) {
            ...
            if (fail > 0 && (!useTeleport || tpMana > me.mp)) {
              ...
              if (
                !leaped &&
                Skill.canUse(sdk.skills.LeapAttack) &&
                Skill.cast(
  ...
  moveNear(x, y, minDist, givenSettings = {}) {
    return Pather.move(
      { x: x, y: y },
  ...
  /*
	...
	*/
  moveTo(x, y, retry, clearPath = false, pop = false) {
    return Pather.move(
      { x: x, y: y },
      { retry: retry, pop: pop, allowClearing: clearPath }
    );
  },

  moveToEx(x, y, givenSettings = {}) {
  ...
  walkTo(x, y, minDist) {
    ...
    if (!me.inTown) {
      ...
      if (me.staminaPercent <= 20) {
        let stam = me
          .getItemsEx(-1, sdk.items.mode.inStorage)
          .filter(
            (i) => i.classid === sdk.items.StaminaPotion && i.isInInventory
          )
          .first();
      ...
      if (
        Skill.canUse(sdk.skills.Charge) &&
        me.paladin &&
        me.mp >= 9 &&
        getDistance(me.x, me.y, x, y) > 8 &&
        ...
        if (Skill.canUse(sdk.skills.Vigor)) {
          ...
          !Config.Vigor &&
          Attack.auradin &&
          Skill.canUse(sdk.skills.HolyFreeze)
        ) {
    ...
    MainLoop: while (getDistance(me.x, me.y, x, y) > minDist && !me.dead) {
      ...
      if (
        ...
        CollMap.checkColl(
          me,
          { x: x, y: y },
          sdk.collision.BlockWall | sdk.collision.ClosedDoor
        )
      ) {
  ...
  openDoors(x, y) {
    ...
    if (door) {
      do {
        if (
          (getDistance(door, x, y) < 4 && door.distance < 9) ||
          door.distance < 4
        ) {
          for (let i = 0; i < 3; i++) {
            Misc.click(0, 0, door);
            ...
            if (
              Misc.poll(() => door.mode === sdk.objects.mode.Active, 1000, 30)
            ) {
              return true;
            }
    ...
    if ([sdk.areas.Harrogath, sdk.areas.BloodyFoothills].includes(me.area)) {
      ...
      if (gate) {
        if (
          (getDistance(gate, x, y) < 4 && gate.distance < 9) ||
          gate.distance < 4
        ) {
          for (let i = 0; i < 3; i++) {
            Misc.click(0, 0, gate);
    ...
    if (monstadoor) {
      do {
        if (
          (monstadoor.hp > 0 &&
            getDistance(monstadoor, x, y) < 4 &&
            monstadoor.distance < 9) ||
          monstadoor.distance < 4
        ) {
          for (let p = 0; p < 20 && monstadoor.hp; p++) {
            Skill.cast(
              Config.AttackSkill[1],
              Skill.getHand(Config.AttackSkill[1]),
              monstadoor
            );
          }
        }
    ...
    if (monstawall) {
      do {
        if (
          (monstawall.hp > 0 &&
            getDistance(monstawall, x, y) < 4 &&
            monstawall.distance < 9) ||
          monstawall.distance < 4
        ) {
  ...
  kickBarrels(x, y) {
    ...
    let barrels = Game.getUnits(sdk.unittype.Object).filter(function (el) {
      return (
        el.name &&
        el.mode === sdk.objects.mode.Inactive &&
        [
          "ratnest",
          "goo pile",
          "barrel",
  ...
  /*
		...
		pop - remove last node
	*/
  moveToUnit(unit, offX, offY, clearPath, pop) {
    ...
    //iomars hasOwnProperty
    if (!unit || !("x" in unit) || !("y" in unit))
      throw new Error("moveToUnit: Invalid unit.");

    unit instanceof PresetUnit &&
    ...
    return this.moveTo(
      unit.x + offX,
      unit.y + offY,
      useTeleport && unit.type && unit.isMonster ? 3 : 0,
      clearPath,
      pop
    );
  },

  moveNearUnit(unit, minDist, clearPath, pop = false) {
    const useTeleport = this.useTeleport();
    minDist === undefined && (minDist = me.inTown ? 2 : 5);

    //iomars hasOwnProperty
    if (!unit || !("x" in unit) || !("y" in unit))
      throw new Error("moveNearUnit: Invalid unit.");
    ...
    if (!useTeleport) {
      // The unit will most likely be moving so call the first walk with 'pop' parameter
      this.moveNear(unit.x, unit.y, minDist, {
        clearSettings: { clearPath: clearPath },
        pop: true,
      });
    }

    return this.moveNear(unit.x, unit.y, minDist, {
      clearSettings: { clearPath: clearPath },
      pop: pop,
    });
  },
  ...
  moveNearPreset(
    ...
    if (!presetUnit) {
      throw new Error(
        "moveNearPreset: Couldn't find preset unit - id: " +
          unitId +
          " unitType: " +
          unitType +
          " in area: " +
          this.getAreaName(area)
      );
    }
  ...
  moveToPreset(area, unitType, unitId, offX, offY, clearPath, pop) {
    ...
    if (!presetUnit) {
      throw new Error(
        "moveToPreset: Couldn't find preset unit - id: " +
          unitId +
          " unitType: " +
          unitType +
          " in area: " +
    ...
    return this.moveTo(
      presetUnit.roomx * 5 + presetUnit.x + offX,
      presetUnit.roomy * 5 + presetUnit.y + offY,
      3,
      clearPath,
      pop
    );
  ...
  /*
		...
		clearPath - kill monsters while moving
	*/
  moveToExit(targetArea, use = false, clearPath = false) {
    ...
    console.info(
      ...
      "\xFFc7MyArea: \xFFc0" +
        ...
        " \xFFc7TargetArea: \xFFc0" +
        Pather.getAreaName(targetArea)
    );
    ...
    for (let i = 0; i < areas.length; i += 1) {
      ...
      Config.DebugMode &&
        console.log("Took: " + (getTickCount() - t2) + " to assign vars");
      ...
      Config.DebugMode &&
        console.log("Took: " + (getTickCount() - t3) + " to find all exits");
      ...
      if (checkExits.length > 0) {
        ...
        Config.DebugMode &&
          console.log(
            "Took: " + (getTickCount() - t4) + " to pick exit",
            currExit
          );
        let t5 = getTickCount();
        ...
        Config.DebugMode &&
          console.log(
            "Took: " + (getTickCount() - t5) + " to find nearest walkable"
          );
        ...
        for (let retry = 0; retry < 3; retry++) {
          ...
          delay(200);
          console.log("\xFFc7(moveToExit) :: \xFFc0Retry: " + (retry + 1));
        ...
        if (use || i < areas.length - 1) {
          switch (currExit.type) {
            ...
            case 2: // stairs
              if (
                !this.openExit(areas[i]) &&
                !this.useUnit(sdk.unittype.Stairs, currExit.tileid, areas[i])
              ) {
                return false;
              }
    ...
    console.info(
      false,
      "\xFFc7targetArea: \xFFc0" +
        this.getAreaName(finalDest) +
        " \xFFc7myArea: \xFFc0" +
        this.getAreaName(me.area),
      "moveToExit"
    );
    delay(300);
  ...
  getNearestRoom(area) {
    ...
    if (room) {
      ...
      !!Config.DebugMode &&
        console.log(
          "\xFFc7End \xFFc8(getNearestOld) \xFFc0 - \xFFc7Duration: \xFFc0" +
  ...
  openExit(targetArea) {
    switch (true) {
      ...
      case targetArea === sdk.areas.A2SewersLvl1 &&
        !(me.inArea(sdk.areas.LutGholein) && [5218, 5180].distance < 20):
        return this.useUnit(
          sdk.unittype.Object,
          sdk.objects.TrapDoorA2,
          targetArea
        );
      ...
      case targetArea === sdk.areas.DuranceOfHateLvl1 &&
        me.inArea(sdk.areas.Travincal):
        return this.useUnit(
          sdk.unittype.Object,
          sdk.objects.DuranceEntryStairs,
          targetArea
        );
      case targetArea === sdk.areas.WorldstoneLvl1 &&
        me.inArea(sdk.areas.ArreatSummit):
        return this.useUnit(
          sdk.unittype.Object,
  ...
  useUnit(type, id, targetArea) {
    let unit = Misc.poll(() => getUnit(type, id), 2000, 200);
    let preArea = me.area;

    if (!unit) {
      throw new Error(
        "useUnit: Unit not found. TYPE: " +
          type +
          " ID: " +
          id +
          " MyArea: " +
          this.getAreaName(me.area) +
          (!!targetArea ? " TargetArea: " + Pather.getAreaName(targetArea) : "")
      );
    }
    ...
    for (let i = 0; i < 5; i += 1) {
      ...
      if (
        type === sdk.unittype.Object &&
        unit.mode === sdk.objects.mode.Inactive
      ) {
        if (
          (me.inArea(sdk.areas.Travincal) &&
            targetArea === sdk.areas.DuranceofHateLvl1 &&
            me.getQuest(
              sdk.quest.id.TheBlackenedTemple,
              sdk.quest.states.Completed
            ) !== 1) ||
          (me.inArea(sdk.areas.ArreatSummit) &&
            targetArea === sdk.areas.WorldstoneLvl1 &&
            me.getQuest(
          ...
          throw new Error(
            "useUnit: Incomplete quest." +
              (!!targetArea
                ? " TargetArea: " + Pather.getAreaName(targetArea)
                : "")
          );
        }
  ...
  /*
		...
		targetArea - area id of where the unit leads to
	*/
  useUnitEx(givenSettings = {}, targetArea = undefined) {
    ...
    let unit = settings.unit
      ...
      ? Misc.poll(() => getUnit(settings.type, settings.id), 2000, 200)
      : null;
  ...
  /*
	...
	*/
  useWaypoint(targetArea, check = false) {
    switch (targetArea) {
      case undefined:
        throw new Error(
          "useWaypoint: Invalid targetArea parameter: " + targetArea
        );
      case null:
      case "random":
        check = true;
    ...
    //iomars checked targetArea
    targetArea && this.broadcastIntent(targetArea);
    console.info(
      true,
      "\xFFc7targetArea: \xFFc0" +
        this.getAreaName(targetArea) +
        " \xFFc7myArea: \xFFc0" +
        this.getAreaName(me.area)
    );
    ...
    for (let i = 0; i < 12; i += 1) {
      ...
      if (me.inTown) {
        if (me.inArea(sdk.areas.LutGholein)) {
          ...
          if (!!npc && npc.distance < 50) {
            if (npc && npc.openMenu()) {
              ...
              if (
                !Misc.poll(
                  () => me.gameReady && me.inArea(sdk.areas.RogueEncampment),
                  2000,
                  100
                )
              ) {
        ...
        !getUIFlag(sdk.uiflags.Waypoint) &&
          Town.getDistance("waypoint") > (Skill.haveTK ? 20 : 5) &&
          Town.move("waypoint");
      }
      ...
      if (!!wp && wp.area === me.area) {
        ...
        if (check || Config.WaypointMenu || !this.initialized) {
          ...
          while (
            getTickCount() - tick <
            Math.max(Math.round(((i + 1) * 1000) / (i / 5 + 1)), pingDelay * 2)
          ) {
            ...
            if (getUIFlag(sdk.uiflags.Waypoint)) {
              ...
              switch (targetArea) {
                case "random":
                  while (true) {
                    targetArea =
                      this.nonTownWpAreas[
                        rand(0, this.nonTownWpAreas.length - 1)
                      ];
              ...
              if (!getWaypoint(this.wpAreas.indexOf(targetArea))) {
                me.cancel();
                console.log(
                  "Trying to get the waypoint: " + this.getAreaName(targetArea)
                );
                ...
                throw new Error(
                  "Pather.useWaypoint: Failed to go to waypoint " +
                    this.getAreaName(targetArea)
                );
              }
          ...
          if (!getUIFlag(sdk.uiflags.Waypoint)) {
            ...
            let coord = CollMap.getRandCoordinate(
              me.x,
              -5 * retry,
              5 * retry,
              me.y,
        ...
        if (!check || getUIFlag(sdk.uiflags.Waypoint)) {
            ...
            if (me.area === targetArea) {
              ...
              console.info(
                ...
                "\xFFc7targetArea: \xFFc0" +
                  ...
                  " \xFFc7myArea: \xFFc0" +
                  this.getAreaName(me.area) +
                  "\xFFc0 - \xFFc7Duration: \xFFc0" +
                  Time.format(getTickCount() - wpTick)
              );
          ...
          Misc.poll(() => me.gameReady, 2000, 100) &&
            getUIFlag(sdk.uiflags.Waypoint) &&
            me.cancelUIFlags();
        }
  ...
  /*
	...
	*/
  makePortal(use = false, force = false) {
    if (me.inTown) return true;

    //iomars
    if (!Town.canTpToTown(force)) return false;
    ...
    for (let i = 0; i < 5; i += 1) {
      ...
      let oldPortal = Game.getUnits(sdk.unittype.Object, "portal")
      ...
      if (tpTool.use() || Game.getObject("portal")) {
        ...
        while (
          getTickCount() - tick <
          Math.max(500 + i * 100, pingDelay * 2 + 100)
        ) {
          let portal = Game.getUnits(sdk.unittype.Object, "portal")
  ...
  /*
	...
	*/
  usePortal(targetArea, owner, unit) {
    // while (!me.gameReady) {
    // 	delay(30);
    // }
      ...
      if (portal) {
        ...
        if (portal.area === me.area) {
          if (Skill.useTK(portal) && i < 3) {
            ...
            if (
              Skill.cast(sdk.skills.Telekinesis, sdk.skills.hand.Right, portal)
            ) {
              if (
                Misc.poll(
                  () => {
            ...
            if (getTickCount() - this.lastPortalTick > 2500) {
              ...
              let timeTillNextPortal = Math.max(
                3,
                Math.round(2500 - (getTickCount() - this.lastPortalTick))
              );
        ...
        if (
          portal.classid === sdk.objects.ArcaneSanctuaryPortal &&
          portal.mode !== sdk.objects.mode.Active
        ) {
        ...
        if (typeof targetArea !== "number" || portal.objtype === targetArea) {
          switch (owner) {
            ...
            default: // Pather.usePortal(null, owner) - any blue portal belonging to owner OR Pather.usePortal(area, owner) - blue portal matching area and owner
              if (
                portal.getParent() === owner &&
                (owner === me.name || Misc.inMyParty(owner))
              ) {
                return copyUnit(portal);
              }
  ...
  /*
	...
	*/
  getNearestWalkable(x, y, range, step, coll, size) {
    !step && (step = 1);
    ...
    MainLoop: while (!result && distance < range) {
      for (let i = -distance; i <= distance; i += 1) {
        for (let j = -distance; j <= distance; j += 1) {
          ...
          if (
            Math.abs(i) >= Math.abs(distance) ||
            Math.abs(j) >= Math.abs(distance)
          ) {
  ...
  /*
	...
	*/
  checkSpot(x, y, coll, cacheOnly, size) {
    coll === undefined && (coll = sdk.collision.BlockWall);
    !size && (size = 1);
      ...
      if (preset) {
        ...
        if (wp) {
          for (let j = 0; j < 10; j++) {
            ...
            if (
              Misc.poll(
                () => me.gameReady && getUIFlag(sdk.uiflags.Waypoint),
                1000,
                150
              )
            ) {
  ...
  journeyTo(area) {
    ...
    if (area !== sdk.areas.DurielsLair) {
      ...
      target = {
        course: [sdk.areas.CanyonofMagic, sdk.areas.DurielsLair],
        useWP: false,
      };
    ...
    area === sdk.areas.PandemoniumFortress &&
      me.inArea(sdk.areas.DuranceofHateLvl3) &&
      (target.useWP = false);
      ...
      if (special) {
        ...
        for (let i = 0; i < special.length; i += 1) {
          if (special[i].target === sdk.areas.GreatMarsh) {
            ...
            target.course.splice(
              target.course.indexOf(sdk.areas.FlayerJungle),
              0,
              sdk.areas.GreatMarsh
            );
    ...
    while (target.course.length) {
      ...
      if (!me.inTown) {
        ...
        if (
          this.wpAreas.includes(currArea) &&
      ...
      if (
        me.inTown &&
        this.nextAreas[currArea] !== targetArea &&
        this.wpAreas.includes(targetArea) &&
        getWaypoint(this.wpAreas.indexOf(targetArea))
      ) {
        this.useWaypoint(targetArea, !this.plotCourse_openedWpMenu);
        Precast.doPrecast(false);
      } else if (
        currArea === sdk.areas.StonyField &&
        targetArea === sdk.areas.Tristram
      ) {
        ...
        currArea === sdk.areas.LutGholein &&
        targetArea === sdk.areas.A2SewersLvl1
      ) {
        // Lut Gholein -> Sewers Level 1 (use Trapdoor)
        this.moveToPreset(
          currArea,
          sdk.unittype.Stairs,
          sdk.exits.preset.A2SewersTrapDoor
        );
        this.useUnit(
          sdk.unittype.Object,
          sdk.objects.TrapDoorA2,
        ...
        currArea === sdk.areas.A2SewersLvl2 &&
        targetArea === sdk.areas.A2SewersLvl1
      ) {
        ...
        this.useUnit(
          sdk.unittype.Stairs,
          sdk.objects.A2UndergroundUpStairs,
          sdk.areas.A2SewersLvl1
        );
      } else if (
        currArea === sdk.areas.PalaceCellarLvl3 &&
        targetArea === sdk.areas.ArcaneSanctuary
      ) {
        // Palace -> Arcane
        this.moveTo(10073, 8670);
        this.usePortal(null);
      } else if (
        currArea === sdk.areas.ArcaneSanctuary &&
        targetArea === sdk.areas.PalaceCellarLvl3
      ) {
        ...
        unit = Misc.poll(() =>
          Game.getObject(sdk.objects.ArcaneSanctuaryPortal)
        );
        unit &&
          Pather.useUnit(
            sdk.unittype.Object,
        ...
        currArea === sdk.areas.ArcaneSanctuary &&
        targetArea === sdk.areas.CanyonofMagic
      ) {
        ...
        currArea === sdk.areas.CanyonofMagic &&
        targetArea === sdk.areas.DurielsLair
      ) {
        // Canyon -> Duriels Lair
        ...
        this.moveToPreset(
          me.area,
          sdk.unittype.Object,
          sdk.objects.HoradricStaffHolder
        );
        ...
        currArea === sdk.areas.DuranceofHateLvl3 &&
        targetArea === sdk.areas.PandemoniumFortress
      ) {
        // Durance Lvl 3 -> Pandemonium Fortress
        if (
          me.getQuest(sdk.quest.id.TheGuardian, sdk.quest.states.Completed) !==
          1
        ) {
        ...
        currArea === sdk.areas.Harrogath &&
        targetArea === sdk.areas.BloodyFoothills
      ) {
        ...
        currArea === sdk.areas.Harrogath &&
        targetArea === sdk.areas.NihlathaksTemple
      ) {
        // Harrogath -> Nihlathak's Temple
        Town.move(NPC.Anya);
        if (
          !Pather.getPortal(sdk.areas.NihlathaksTemple) &&
          Misc.checkQuest(
        ...
        currArea === sdk.areas.FrigidHighlands &&
        targetArea === sdk.areas.Abaddon
      ) {
        // Abaddon
        this.moveToPreset(
          sdk.areas.FrigidHighlands,
        ...
        currArea === sdk.areas.ArreatPlateau &&
        targetArea === sdk.areas.PitofAcheron
      ) {
        // Pits of Archeon
        this.moveToPreset(
          sdk.areas.ArreatPlateau,
        ...
        currArea === sdk.areas.FrozenTundra &&
        targetArea === sdk.areas.InfernalPit
      ) {
        // Infernal Pit
        this.moveToPreset(
          sdk.areas.FrozenTundra,
    ...
    console.info(
      false,
      "\xFFc4MyArea: \xFFc0" + Pather.getAreaName(me.area),
      "journeyTo"
    );
    return me.area === area;
  },
  ...
  plotCourse(dest, src) {
    ...
    if (
      !this.plotCourse_openedWpMenu &&
      me.inTown &&
      this.nextAreas[me.area] !== dest &&
      this.useWaypoint(null)
    ) {
    ...
    while (toVisitNodes.length > 0) {
      ...
      if (visitedNodes[node.from] === undefined) {
        ...
        if (this.areasConnected(node.from, node.to)) {
          ...
          if (
            (me.inTown || // check wp in town
              (src !== previousAreas[dest] &&
                dest !== previousAreas[src] && // check wp if areas aren't linked
                previousAreas[src] !== previousAreas[dest])) && // check wp if areas aren't linked with a common area
            Pather.wpAreas.indexOf(node.from) > 0 &&
            getWaypoint(Pather.wpAreas.indexOf(node.from))
          ) {
            if (node.from !== src) {
              useWP = true;
            }
          ...
          if (
            (prevArea = previousAreas[node.from]) !== 0 &&
            visitedNodes.indexOf(prevArea) === -1
          ) {
          ...
          for (prevArea = 1; prevArea < previousAreas.length; prevArea += 1) {
            ...
            if (
              previousAreas[prevArea] === node.from &&
              visitedNodes.indexOf(prevArea) === -1
            ) {
              toVisitNodes.push({ from: prevArea, to: node.from });
            }
  ...
  getAreaId(area) {
    if (["number", "string"].indexOf(typeof area) === -1) return 0;
    if (typeof area === "number") return area;

    return this.areaNames.indexOf(area) === -1
      ? 0
      : this.areaNames.indexOf(area);
  },
};
