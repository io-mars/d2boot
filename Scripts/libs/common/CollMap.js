...
export const CollMap = {
  ...
  getNearbyRooms(x, y) {
    ...
    for (let i = 0; i < rooms.length; i += 1) {
      if (
        this.getRoomIndex(
          rooms[i].x * 5 + rooms[i].xsize / 2,
          rooms[i].y * 5 + rooms[i].ysize / 2,
          true
        ) === undefined
      ) {
  ...
  getColl(x, y, cacheOnly) {
    ...
    if (
      this.maps[index] !== undefined &&
      this.maps[index][i] !== undefined &&
      this.maps[index][i][j] !== undefined
    ) {
  ...
  coordsInRoom(x, y, room) {
    if (
      room &&
      x >= room.x * 5 &&
      x < room.x * 5 + room.xsize &&
      y >= room.y * 5 &&
      y < room.y * 5 + room.ysize
    ) {
      return true;
    }
  ...
  checkColl(unitA, unitB, coll, thickness) {
  ...
  getTelePoint(room) {
    ...
    if (getCollision(room.area, roomx, roomy) & 1) {
      let collision = room.getCollision(),
        validTiles = [];
      let aMid = Math.round(collision.length / 2),
        bMid = Math.round(collision[0].length / 2);
  ...
  getRandCoordinate(cX, xmin, xmax, cY, ymin, ymax, factor = 1) {
    ...
    do {
      ...
      if (cX === coordX && cY === coordY) {
        // recalculate if same coordiante
        coordX = 0;
        continue;
      }
