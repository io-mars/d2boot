...
export const Attack = {
  ...
  async asyncInit(notify = true) {
    ...
    if (notify && (Config.AttackSkill[1] < 0 || Config.AttackSkill[3] < 0)) {
      showConsole();
      console.warn("\xFFc1Bad attack config. Don't expect your bot to attack.");
    }
  ...
  checkSlot(slot = me.weaponswitch) {
    ...
    if (item) {
      do {
        if (me.weaponswitch !== slot) {
          if (
            item.bodylocation === sdk.body.RightArmSecondary ||
            item.bodylocation === sdk.body.LeftArmSecondary
          ) {
            return true;
          }
        } else {
    ...
    if (Config.PrimarySlot === -1) {
      if (me.classic) {
        ...
        if (Precast.haveCTA > -1 || Precast.checkCTA()) {
          ...
          !this.checkSlot(sdk.player.slot.Main) &&
          this.checkSlot(sdk.player.slot.Secondary)
        ) {
  ...
  checkInfinity() {
    ...
    Config.UseMerc &&
      !me.mercrevivecost &&
      (merc = Misc.poll(() => me.getMerc(), 1000, 100));

    // Check merc infinity
    !!merc &&
      (this.infinity = merc.checkItem({
        name: sdk.locale.items.Infinity,
      }).have);

    // Check player infinity - only check if merc doesn't have
    !this.infinity &&
      (this.infinity = me.checkItem({
        name: sdk.locale.items.Infinity,
        equipped: true,
      }).have);
  ...
  kill(classId) {
    ...
    console.log("\xFFc7Kill \xFFc0:: " + who);
    Config.MFLeader && Pather.makePortal() && say(`kill ${classId}@${me.area}`);

    while (
      attackCount < Config.MaxAttackCount &&
      target.attackable &&
      !this.skipCheck(target)
    ) {
      Misc.townCheck();
      ...
      if (!target || !copyUnit(target).x) {
        ...
        if (!target) {
          console.warn(
            "\xFFc1Failed to kill " + who + " (couldn't relocate unit)"
          );
          break;
        }
      }
      ...
      let result = this.ClassAttack.doAttack(target, attackCount % 15 === 0);
    ...
    attackCount === Config.MaxAttackCount &&
      (errorInfo = " (attackCount exceeded: " + attackCount + ")");
    Config.MFSwitchPercent && me.switchWeapons(this.getPrimarySlot());
    this.ClassAttack.afterAttack();
    try {
      Pickit.pickItems();
    } catch (error) {
      console.error(error);
    ...
    if (!!target && target.attackable) {
      console.warn("\xFFc1Failed to kill \xFFc0" + who + errorInfo);
    } else {
      console.log(
        "\xFFc7Killed \xFFc0:: " +
          who +
  ...
  getScarinessLevel(unit) {
    ...
    const ids = [
      ...
      sdk.monsters.HollowOne,
      sdk.monsters.Guardian1,
      sdk.monsters.Guardian2,
      sdk.monsters.Unraveler1,
      sdk.monsters.Unraveler2,
      sdk.monsters.Ancient1,
      sdk.monsters.Ancient2,
      sdk.monsters.Ancient3,
      sdk.monsters.BaalSubjectMummy,
      sdk.monsters.RatManShaman,
      sdk.monsters.FetishShaman,
      sdk.monsters.FlayerShaman1,
      sdk.monsters.FlayerShaman2,
      sdk.monsters.SoulKillerShaman1,
      sdk.monsters.SoulKillerShaman2,
      sdk.monsters.StygianDollShaman1,
      sdk.monsters.StygianDollShaman2,
      sdk.monsters.FleshSpawner1,
      sdk.monsters.FleshSpawner2,
      sdk.monsters.StygianHag,
      sdk.monsters.Grotesque1,
  ...
  clear(range, spectype, bossId, sortfunc, pickit = true) {
    while (!me.gameReady) {
      delay(40);
    }
      ...
      do {
        if (
          ...
          if (
            ...
            (Pather.canTeleport() ||
              !checkCollision(me, target, sdk.collision.WallOrRanged))
          ) {
            start = true;
          }
    ...
    while (
      ...
      if (
        ...
        if (!logged && boss && boss.gid === target.gid) {
          logged = true;
          console.log(
            "\xFFc7Clear \xFFc0:: " + (!!target.name ? target.name : bossId)
          );
        }
        ...
        if (result) {
          ...
          if (
            Config.AttackSkill[secAttack] > -1 &&
            (!Attack.checkResist(
              target,
              Config.AttackSkill[isSpecial ? 1 : 3]
            ) ||
              (me.paladin &&
                Config.AttackSkill[isSpecial ? 1 : 3] ===
                  sdk.skills.BlessedHammer &&
                !this.ClassAttack.getHammerPosition(target)))
          ) {
          ...
          switch (skillCheck) {
            case sdk.skills.BlessedHammer:
              ...
              if (
                gidAttack[i].attacks > 0 &&
                gidAttack[i].attacks % (isSpecial ? 4 : 2) === 0
              ) {
            ...
            default:
              ...
              if (
                gidAttack[i].attacks > 0 &&
                gidAttack[i].attacks % (isSpecial ? 15 : 5) === 0 &&
          ...
          if (target.dead || Config.FastPick) {
            if (boss && boss.gid === target.gid && target.dead) {
              ...
              console.log(
                "\xFFc7Cleared \xFFc0:: " +
                  (!!target.name ? target.name : bossId) +
                  "\xFFc0 - \xFFc7Duration: \xFFc0" +
                  Time.format(getTickCount() - tick)
              );
            }
    ...
    if (attackCount > 0) {
      this.ClassAttack.afterAttack(pickit);
    ...
    if (boss && !killedBoss) {
      ...
      if (boss.dead) {
        console.log(
          "\xFFc7Cleared \xFFc0:: " +
            (!!boss.name ? boss.name : bossId) +
            "\xFFc0 - \xFFc7Duration: \xFFc0" +
            Time.format(getTickCount() - tick)
        );
      } else {
        console.log(
          "\xFFc7Clear \xFFc0:: \xFFc1Failed to clear \xFFc0:: " +
            (!!boss.name ? boss.name : bossId)
        );
      }
    }
  ...
  getMob(classid, spectype, range, center) {
    let monsterList = [];
    let monster = Game.getMonster();
    ...
    switch (typeof classid) {
      ...
      case "string":
        ...
        if (monster) {
          do {
            if (
              getDistance(center.x, center.y, monster.x, monster.y) <= range &&
              (!spectype || monster.spectype & spectype) &&
              monster.attackable
            ) {
      ...
      case "object":
        ...
        if (monster) {
          do {
            if (
              classid.includes(monster.classid) &&
              getDistance(center.x, center.y, monster.x, monster.y) <= range &&
              (!spectype || monster.spectype & spectype) &&
              monster.attackable
            ) {
  ...
  clearList(mainArg, sortFunc, refresh) {
        ...
        if (result) {
          ...
          switch (Config.AttackSkill[isSpecial ? 1 : 3]) {
            case sdk.skills.BlessedHammer:
              ...
              if (
                gidAttack[i].attacks > 0 &&
                gidAttack[i].attacks % (isSpecial ? 5 : 15) === 0
              ) {
                let coord = CollMap.getRandCoordinate(
                  me.x,
                  -1,
    ...
    if (attackCount > 0) {
      this.ClassAttack.afterAttack(true);
  ...
  securePosition(
    ...
    while (true) {
      ...
      if (monster) {
        do {
          if (
            getDistance(monster, x, y) <= range &&
            monster.attackable &&
            this.canAttack(monster) &&
            (!skipBlocked || !checkCollision(me, monster, skipBlocked)) &&
            (Pather.canTeleport() ||
              !checkCollision(me, monster, sdk.collision.BlockWall))
          ) {
      ...
      if (special) {
        if (
          me.paladin &&
          Skill.canUse(sdk.skills.Redemption) &&
    ...
    if (monster) {
      do {
        if (
          ...
          this.ignoredGids.indexOf(monster.gid) === -1
        ) {
          this.uniques += 1;
  ...
  storeStatistics(area) {
    !FileTools.exists("statistics.json") &&
      Misc.fileAction("statistics.json", Misc.FileActionMode.write, "{}");
    ...
    if (obj) {
      ...
      obj[area].averageUniques = (
        (obj[area].averageUniques * obj[area].runs + this.uniques) /
        (obj[area].runs + 1)
      ).toFixed(4);
      obj[area].runs += 1;
  ...
  clearLevel(spectype = 0) {
    ...
    const breakClearLevelCheck = !!(
      Loader.scriptName() === "MFHelper" &&
      Config.MFHelper.BreakClearLevel &&
      Config.Leader !== ""
    );
  ...
  sortMonsters(unitA, unitB) {
    ...
    if (me.inArea(sdk.areas.CatacombsLvl4)) {
      if (
        unitA.distance < 5 &&
        unitA.classid === sdk.monsters.Andariel &&
    ...
    if (me.inArea(sdk.areas.DuranceofHateLvl3)) {
      if (
        unitA.distance < 5 &&
        unitA.classid === sdk.monsters.Mephisto &&
        !checkCollision(me, unitA, sdk.collision.Ranged)
      )
        return -1;
    }
    ...
    if (me.barbarian) {
      if (
        !Attack.checkResist(
          unitA,
          Attack.getSkillElement(Config.AttackSkill[unitA.isSpecial ? 1 : 3])
        )
      ) {
        return 1;
      }
      ...
      if (
        !Attack.checkResist(
          unitB,
          Attack.getSkillElement(Config.AttackSkill[unitB.isSpecial ? 1 : 3])
        )
      ) {
        return -1;
      }
    ...
    if (
      !me.inArea(sdk.areas.ClawViperTempleLvl2) &&
  ...
  validSpot(x, y, skill = -1, unitid = 0) {
    // Just in case
    ...
    if (
      Skill.missileSkills.includes(skill) ||
      ([sdk.skills.Blizzard, sdk.skills.Meteor].includes(skill) &&
        unitid > 0 &&
        !getBaseStat("monstats", unitid, "flying"))
    ) {
      return true;
    }
    ...
    switch (true) {
      ...
      case Attack.monsterObjects.includes(unitid) &&
        (!!(result & sdk.collision.MonsterIsOnFloor) ||
          !!(result & sdk.collision.MonsterObject)):
      ...
      default:
        ...
        if (result & sdk.collision.BlockWall || result & sdk.collision.Objects)
          return false;
  ...
  findSafeSpot(unit, distance, spread, range) {
    ...
    grid.sort(
      (a, b) =>
        getDistance(b.x, b.y, unit.x, unit.y) -
        getDistance(a.x, a.y, unit.x, unit.y)
    );

    for (let i = 0; i < grid.length; i += 1) {
      if (
        !(
          CollMap.getColl(grid[i].x, grid[i].y, true) & sdk.collision.BlockWall
        ) &&
        !CollMap.checkColl(
          unit,
          { x: grid[i].x, y: grid[i].y },
          sdk.collision.Ranged
        )
      ) {
        let currCount = this.getMonsterCount(
          grid[i].x,
          grid[i].y,
          range,
          monList
  ...
  deploy(unit, distance, spread, range) {
    if (arguments.length < 4)
      throw new Error("deploy: Not enough arguments supplied");
  ...
  getMonsterCount(x, y, range, list) {
    let count = 0;
    let ignored = [sdk.monsters.Diablo]; // why is diablo ignored?

    for (let i = 0; i < list.length; i += 1) {
      if (
        ignored.indexOf(list[i].classid) === -1 &&
        list[i].attackable &&
  ...
  buildGrid(xmin, xmax, ymin, ymax, spread) {
  ...
  skipCheck(unit) {
    ...
    if (
      ...
      Config.SkipException.includes(unit.name)
    ) {
      console.log("\xFFc1Skip Exception: " + unit.name);
      return false;
    }
  ...
  getResist(unit, type) {
    ...
    switch (type) {
      ...
      case "holybolt": // check if a monster is undead
        if (
          getBaseStat("monstats", unit.classid, "lUndead") ||
          getBaseStat("monstats", unit.classid, "hUndead")
        ) {
          return 0;
        }
  ...
  getLowerResistPercent() {
    const calc = (level) =>
      Math.floor(Math.min(25 + (45 * ((110 * level) / (level + 6))) / 100, 70));
    if (Skill.canUse(sdk.skills.LowerResist)) {
      return calc(
        me.getSkill(sdk.skills.LowerResist, sdk.skills.subindex.SoftPoints)
      );
    }
    return 0;
  },

  getConvictionPercent() {
    ...
    if (Skill.canUse(sdk.skills.Conviction)) {
      return calc(
        me.getSkill(sdk.skills.Conviction, sdk.skills.subindex.SoftPoints)
      );
    }
    return 0;
  },
  ...
  checkResist(unit, val, maxres = 100) {
    if (!unit || !unit.type || unit.isPlayer) return true;

    const damageType =
      typeof val === "number" ? this.getSkillElement(val) : val;
    const addLowerRes = !!(
      Skill.canUse(sdk.skills.LowerResist) && unit.curseable
    );

    // Static handler
    if (
      val === sdk.skills.StaticField &&
      this.getResist(unit, damageType) < 100
    ) {
    ...
    if (
      this.infinity &&
      ["fire", "lightning", "cold"].includes(damageType) &&
      unit.getState
    ) {
      if (!unit.getState(sdk.states.Conviction)) {
        if (addLowerRes && !unit.getState(sdk.states.LowerResist)) {
          ...
          return (
            this.getResist(unit, damageType) -
              Math.floor((lowerResPercent + 85) / 5) <
            100
          );
        }
    ...
    if (
      ...
      if (!unit.getState(sdk.states.Conviction)) {
        return (
          this.getResist(unit, damageType) - this.getConvictionPercent() / 5 <
          100
        );
      }
    ...
    if (
      addLowerRes &&
      ["fire", "lightning", "cold", "poison"].includes(damageType) &&
      unit.getState
    ) {
      let lowerResPercent = this.getLowerResistPercent();
      if (!unit.getState(sdk.states.LowerResist)) {
        return (
          this.getResist(unit, damageType) - Math.floor(lowerResPercent / 5) <
          100
        );
      }
    }
    ...
    if (unit.isMonster) {
      ...
      if (unit.isSpecial) {
        if (
          Attack.checkResist(
            unit,
            this.getSkillElement(Config.AttackSkill[1])
          ) ||
        ...
        if (
          Attack.checkResist(
            unit,
            this.getSkillElement(Config.AttackSkill[3])
          ) ||
      ...
      if (Config.AttackSkill.length === 7) {
        return (
          Attack.checkResist(
            unit,
            this.getSkillElement(Config.AttackSkill[5])
          ) ||
  ...
  getIntoPosition(unit, distance, coll, walk) {
    ...
    if (distance < 4 && (!("mode" in unit) || !unit.dead)) {
    ...
    let name = "name" in unit ? unit.name : "";
    let angle = Math.round(
      (Math.atan2(me.y - unit.y, me.x - unit.x) * 180) / Math.PI
    );
    let angles = [
      0, 15, -15, 30, -30, 45, -45, 60, -60, 75, -75, 90, -90, 135, -135, 180,
    ];
    ...
    for (let n = 0; n < 3; n += 1) {
      ...
      for (let i = 0; i < angles.length; i += 1) {
        let cx = Math.round(
          Math.cos(((angle + angles[i]) * Math.PI) / 180) * distance + unit.x
        );
        let cy = Math.round(
          Math.sin(((angle + angles[i]) * Math.PI) / 180) * distance + unit.y
        );
          ...
          if (
            ...
            switch (walk) {
              ...
              case 2:
                if (
                  coords[i].distance < 6 &&
                  !CollMap.checkColl(me, coords[i], sdk.collision.WallOrRanged)
                ) {
                  Pather.walkTo(coords[i].x, coords[i].y, 2);
                } else {
    ...
    !!name && print("\xFFc4Attack\xFFc0: No valid positions for: " + name);
      ...
      do {
        if (monster.attackable && !monster.getParent()) {
          ...
          if (
            ...
            (!settings.skipBlocked ||
              !checkCollision(me, monster, sdk.collision.WallOrRanged)) &&
            (!settings.skipImmune || this.canAttack(monster))
          ) {
            range = distance;
            gid = monster.gid;
          }
  ...
  checkCorpse(unit) {
    if (
      !unit ||
      (unit.mode !== sdk.monsters.mode.Death &&
        unit.mode !== sdk.monsters.mode.Dead)
    )
      return false;
    if (
      unit.classid <= sdk.monsters.BurningDeadArcher2 &&
  ...
  checkNearCorpses(unit, range = 15) {
    let corpses = Game.getUnits(sdk.unittype.Monster).filter(function (corpse) {
  ...
  whirlwind(unit) {
    ...
    let angles = [
      180, 175, -175, 170, -170, 165, -165, 150, -150, 135, -135, 45, -45, 90,
      -90,
    ];
    ...
    for (let i = 0; i < angles.length; i += 1) {
      let coords = [
        Math.round(
          Math.cos(((angle + angles[i]) * Math.PI) / 180) * 4 + unit.x
        ),
        Math.round(
          Math.sin(((angle + angles[i]) * Math.PI) / 180) * 4 + unit.y
        ),
      ];
    ...
    return (
      ...
      Skill.cast(
        sdk.skills.Whirlwind,
        Skill.getHand(sdk.skills.Whirlwind),
        me.x,
        me.y
      )
    );
