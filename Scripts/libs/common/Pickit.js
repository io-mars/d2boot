...
import { Skill, Misc, Game, Packet } from "./Misc.js";
...
export const Pickit = {
  ...
  init(notify) {
    ...
    if (typeof Storage !== "undefined") {
      ...
      Config.MinColumn.forEach((el, index) => {
        el >= this.beltSize &&
          (Config.MinColumn[index] = Math.max(1, this.beltSize - 1));
      });
    }
  },
  ...
  canPick(unit) {
    ...
    switch (unit.itemType) {
      case sdk.items.type.Gold:
        ...
        if (
          me.getStat(sdk.stats.Gold) ===
          me.getStat(sdk.stats.Level) * 10000
        ) {
      ...
      case sdk.items.type.Key:
        ...
        if (myKey && key) {
          do {
            if (
              myKey.isInInventory &&
              myKey.getStat(sdk.stats.Quantity) +
                key.getStat(sdk.stats.Quantity) >
                12
            ) {
              return false;
            }
      ...
      case sdk.items.type.RejuvPotion:
        ...
        for (let i = 0; i < 4; i += 1) {
          if (
            ...
            unit.code.includes(Config.BeltColumn[i])
          ) {
  ...
  checkItem(unit) {
    ...
    if (
      rval.result === Pickit.Result.UNWANTED &&
    ...
    if (
      (unit.classid === sdk.items.runes.Ort ||
        unit.classid === sdk.items.runes.Ral) &&
    ...
    if (CraftingSystem.checkItem(unit))
      return resultObj(Pickit.Result.CRAFTING);
    ...
    if (
      rval.result === Pickit.Result.UNWANTED &&
      !getBaseStat("items", unit.classid, "quest") &&
      !Town.ignoredItemTypes.includes(unit.itemType) &&
      !unit.questItem &&
      (unit.isInInventory ||
      ...
      if (!this.invoLocked) {
        ...
        if (itemValuePerSquare >= 2000) {
          ...
          return resultObj(
            Pickit.Result.TRASH,
            "Valuable LowGold Item: " + itemValue
          );
  ...
  pickItem(unit, status, keptLine, retry = 3) {
    function ItemStats(unit) {
      ...
      this.useTk =
        Skill.haveTK &&
        Pickit.tkable.includes(this.type) &&
        this.dist > 5 &&
        this.dist < 20 &&
    ...
    //iomars let item= Game.getItem(-1, -1, gid) not a good idea, the item is a copyUnit object
    ...
    let tkMana = stats.useTk
      ? Skill.getManaCost(sdk.skills.Telekinesis) * 2
      : Infinity;

    MainLoop: for (let i = 0; i < retry; i += 1) {
      ...
      if (stats.useTk && me.mp > tkMana) {
        ...
        if (
          item.distance > (Config.FastPick || i < 1 ? 6 : 4) ||
          checkCollision(me, item, sdk.collision.BlockWall)
        ) {
          //iomars fixed item pickited, the item have lost bug
          // if (!Pather.moveNearUnit(item, 4)) {
          if (
            !Pather.moveNearUnit(
              { x: item.x, y: item.y, roomx: item.roomx, roomy: item.roomy },
              4
            )
          ) {
            continue;
      ...
      while (getTickCount() - tick < 1000) {
        ...
        if (stats.classid === sdk.items.Gold) {
          if (
            !item.getStat(sdk.stats.Gold) ||
            item.getStat(sdk.stats.Gold) < stats.gold
          ) {
            console.log(
              "\xFFc7Picked up " +
                stats.color +
            ...
            case sdk.items.ScrollofIdentify:
              console.log(
                "\xFFc7Picked up " +
                  ...
                  " \xFFc7(" +
                  Town.checkScrolls(
                    stats.classid === sdk.items.ScrollofTownPortal
                      ? "tbk"
                      : "ibk"
                  ) +
                  "/20)"
      ...
      switch (status) {
        ...
        case Pickit.Result.CUBING:
          ...
          Misc.itemLogger(
            "Kept",
            item,
            "Cubing " + me.findItems(item.classid).length
          );
          Cubing.update();
  ...
  pickItems(range = Config.PickRange) {
    if (me.dead) return false;
    ...
    while (pickList.length > 0) {
      ...
      if (
        copyUnit(itemToPick).x !== undefined &&
        itemToPick.onGroundOrDropping &&
        (Pather.useTeleport() ||
          me.inTown ||
          !checkCollision(me, itemToPick, sdk.collision.BlockWall))
      ) {
        ...
        if (status.result && this.canPick(itemToPick)) {
          ...
          let canFit =
            Storage.Inventory.CanFit(itemToPick) ||
            Pickit.essentials.includes(itemToPick.itemType);

          // Field id when our used space is above a certain percent or if we are full try to make room with FieldID
          if (
            Config.FieldID.Enabled &&
            (!canFit ||
              Storage.Inventory.UsedSpacePercent() > Config.FieldID.UsedSpace)
          ) {
            Town.fieldID() &&
              (canFit =
                itemToPick.gid !== undefined &&
                Storage.Inventory.CanFit(itemToPick));
          }
          ...
          if (!canFit) {
            ...
            if (this.canMakeRoom()) {
              console.log(
                "\xFFc7Trying to make room for " +
                  this.itemColor(itemToPick) +
                  itemToPick.name
              );
    ...
    if (
      needMule &&
      AutoMule.getInfo() &&
      AutoMule.getInfo().hasOwnProperty("muleInfo") &&
      AutoMule.getMuleItems().length > 0
    ) {
