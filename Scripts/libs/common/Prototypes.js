...
if (!Array.prototype.remove) {
  Array.prototype.remove = function (val) {
    ...
    if (val === undefined || !val)
      throw new Error(
        "Cannot remove and element if there is no element defined"
      );
...
if (!Array.prototype.greaterThan) {
  Array.prototype.greaterThan = function (val) {
    if (this === undefined || !this.length || val === undefined || !val.length)
      throw new Error("No Array defined");
    if (this.length !== val.length) throw new Error("must same length Array");

    // return this.reduce(
    //   (accumulator, current, index) =>
    //     accumulator + (current >= val[index] ? 1 : 0),
    //   0
    // );
    return this.every((e, idx) => e >= val[idx]);
  };
}

if (!Array.prototype.consumeThan) {
  Array.prototype.consumeThan = function (val) {
    if (this === undefined || !this.length || val === undefined || !val.length)
      throw new Error("No Array defined");
    if (this.length !== val.length) throw new Error("must same length Array");

    return this.map((el, index) => (el > val[index] ? val[index] : el));
  };
}
...
if (!Array.prototype.addArray) {
  Array.prototype.addArray = function (val) {
    ...
    return this.forEach((el, index) => (this[index] = el + val[index]));
  };
}

if (!Array.prototype.minusArray) {
  Array.prototype.minusArray = function (val) {
    if (this === undefined || !this.length || val === undefined || !val.length)
      throw new Error("No Array defined");
    if (this.length !== val.length) throw new Error("must same length Array");
...
if (!String.prototype.capitalize) {
  String.prototype.capitalize = function (downcase = false) {
    return (
      ...
      (downcase ? this.slice(1).toLowerCase() : this.slice(1))
    );
  };
}

//iomars
if (!String.prototype.omitColor) {
  String.prototype.omitColor = function (trim = false) {
    const re = /(ÿ|\xFF|\?)c[0-9!"+<;:.*]/g;
    return trim ? this.replace(re, "").trim() : this.replace(re, "");
  };
}

// Error.prototype.isNotInterrupted = function () {
//   return this instanceof InternalError && this.message === "interrupted"
//     ? false
//     : true;
// };

if (!Object.hasOwn(Error.prototype, "isInterrupted")) {
  Object.defineProperty(Error.prototype, "isInterrupted", {
    get() {
      return this instanceof InternalError && this.message === "interrupted"
        ? true
        : false;
...
(function (global, print) {
  global.console =
    // iomars must cancel this line! quikjs have set global.console
    // global.console ||
    (function () {
      const console = {};
      const argMap = (el) =>
        (typeof el === "object" && el /*not null */ && JSON.stringify(el)) ||
        el;
      ...
      console.debug = function (...args) {
        if (console.printDebug) {
          ...
          let filenameAndLine, source;
          if (stack && stack[0]) {
            source = stack[0].match(/\sat (.*?) \((.*?\.js):(\d+)\)$/);
          }
          filenameAndLine = source
            ? `${source[1]} (${source[2]}:${source[3]})`
            : "unknown:0";

          this.log(
            "[\xFFc:Debug\xFFc0] \xFFc:[" +
              filenameAndLine +
              "]\xFFc0 :: " +
              args.map(argMap).join(",")
          );
        }
      };
      ...
      console.warn = function (...args) {
        ...
        let filenameAndLine =
          (
            ...
            stack.length &&
            stack[0]
              .substring(stack[0].lastIndexOf("\\") + 1)
              .replace(/libs\/common\//g, "")
          ).trim() || "unknown:0";
        filenameAndLine = filenameAndLine.replace(":", " :: ");
        this.log(
          "[\xFFc9Warning\xFFc0] \xFFc9[" +
            filenameAndLine +
            "]\xFFc0 " +
            args.map(argMap).join(",")
        );
      ...
      console.error = function (error = "") {
        if (error.isInterrupted) return;

        if (typeof error === "string") {
          this.log("[\xFFc1Error\xFFc0] " + error);
        } else {
          let filenameAndLine, line, source;
          if (error && error.stack) {
            line = error.stack
              .split(/\n/)
              .find((item) => item.match(/\sat (.*?) \((.*?\.js):(\d+)\)$/));

            line && (source = line.match(/\sat (.*?) \((.*?\.js):(\d+)\)$/));
          }
      ...
      console.timeEnd = function (name) {
        ...
        if (currTimer) {
          this.log(
            "[\xFFc7Timer\xFFc0] :: \xFFc8" +
              name +
              " - \xFFc4Duration\xFFc0: " +
              (getTickCount() - currTimer) +
              "ms"
          );
          delete timers[name];
        }
      ...
      console.trace = function () {
        ...
        if (stack) {
          ...
          for (let i = 0; i < stack.length - 1; i += 1) {
            if (stack[i]) {
              stackLog +=
                stack[i].substring(stack[i].indexOf("@") + 1) +
                stack[i].substring(
                  stack[i].lastIndexOf("\\") + 1,
                  stack[i].length - 1
                );
          ...
          this.log("[\xFFc;StackTrace\xFFc0] :: " + stackLog);
        }
      };

      console.info = function (start = false, msg = "", timer = "") {
        let stack = new Error().stack.match(/[^\r\n]+/g);
        let funcName = stack[1]
          .substring(stack[1].indexOf("at") + 3)
          .replace(/libs\/common\//g, "");
        let logInfo =
          start === true
            ? "[\xFFc2Start "
            ...
            ? "[\xFFc1End "
            : "[\xFFc8";
        logInfo += funcName + "\xFFc0] :: " + (msg ? msg : "");
        if (timer) {
          ...
          if (currTimer) {
            ...
            tFormat > 1000
              ? (tFormat = Time.format(tFormat))
              : (tFormat += " ms");
            logInfo += " - \xFFc4Duration\xFFc0: " + tFormat;
            delete timers[timer];
          }
        }
        this.log(logInfo);
...
(function (global, original) {
  ...
  global.getUnit = function (...args) {
    ...
    let [first] = args,
      second = args.length >= 2 ? args[1] : undefined;
...
Unit.prototype.__defineGetter__("idle", function () {
  ...
  return (
    this.mode === sdk.player.mode.StandingOutsideTown ||
    this.mode === sdk.player.mode.StandingInTown ||
    this.mode === sdk.player.mode.Dead
  );
});
...
Unit.prototype.__defineGetter__("dead", function () {
  switch (this.type) {
    case sdk.unittype.Player:
      return (
        this.mode === sdk.player.mode.Death ||
        this.mode === sdk.player.mode.Dead
      );
    case sdk.unittype.Monster:
      return (
        this.mode === sdk.monsters.mode.Death ||
        this.mode === sdk.monsters.mode.Dead
      );
    default:
      return false;
  }
...
Unit.prototype.__defineGetter__("attacking", function () {
  if (this.type > sdk.unittype.Monster)
    throw new Error(
      "Unit.attacking: Must be used with Monster or Player units."
    );
  switch (this.type) {
    case sdk.unittype.Player:
      return [
        sdk.player.mode.Attacking1,
        sdk.player.mode.Attacking2,
        sdk.player.mode.CastingSkill,
        sdk.player.mode.ThrowingItem,
        sdk.player.mode.Kicking,
        sdk.player.mode.UsingSkill1,
        sdk.player.mode.UsingSkill2,
        sdk.player.mode.UsingSkill3,
        sdk.player.mode.UsingSkill4,
        sdk.player.mode.SkillActionSequence,
      ].includes(this.mode);
    case sdk.unittype.Monster:
      return [
        sdk.monsters.mode.Attacking1,
        sdk.monsters.mode.Attacking2,
        sdk.monsters.mode.CastingSkill,
        sdk.monsters.mode.UsingSkill1,
        sdk.monsters.mode.UsingSkill2,
        sdk.monsters.mode.UsingSkill3,
        sdk.monsters.mode.UsingSkill4,
      ].includes(this.mode);
    default:
      return false;
  }
...
Unit.prototype.__defineGetter__("durabilityPercent", function () {
  if (this.type !== sdk.unittype.Item)
    throw new Error("Unit.durabilityPercent: Must be used on items.");
  if (
    this.getStat(sdk.stats.Quantity) ||
    !this.getStat(sdk.stats.MaxDurability)
  )
    return 100;
  return Math.round(
    (this.getStat(sdk.stats.Durability) * 100) /
      this.getStat(sdk.stats.MaxDurability)
  );
});
...
Unit.prototype.buy = function (shiftBuy, gamble) {
  ...
  if (
    !getUIFlag(sdk.uiflags.Shop) ||
    (this.getParent() && this.getParent().gid !== getInteractedNPC().gid)
  ) {
...
// Item owner name
Unit.prototype.__defineGetter__("parentName", function () {
  if (this.type !== sdk.unittype.Item)
...
Unit.prototype.drop = function () {
  if (this.type !== sdk.unittype.Item)
    throw new Error(
      "Unit.drop: Must be used with items. Unit Name: " + this.name
    );
...
Unit.prototype.use = function () {
  ...
  if (this.type !== sdk.unittype.Item)
    throw new Error(
      "Unit.use: Must be used with items. Unit Name: " + this.name
    );
  if (!getBaseStat("items", this.classid, "useable"))
    throw new Error(
      "Unit.use: Must be used with consumable items. Unit Name: " + this.name
    );
  ...
  if (checkQuantity) {
    return Misc.poll(
      () => this.getStat(sdk.stats.Quantity) < quantity,
      200 + pingDelay,
      50
    );
  } else {
...
me.findItem = function (id = -1, mode = -1, loc = -1, quality = -1) {
  ...
  if (item) {
    do {
      if (
        (loc === -1 || item.location === loc) &&
        (quality === -1 || item.quality === quality)
      ) {
        return item;
      }
    } while (item.getNext());
...
me.cancelUIFlags = function () {
  ...
  const flags = [
    ...
    sdk.uiflags.Stash,
    sdk.uiflags.Cube,
    sdk.uiflags.KeytotheCairnStonesScreen,
    sdk.uiflags.SubmitItem,
  ];
...
me.switchWeapons = function (slot) {
  if (
    this.gametype === sdk.game.gametype.Classic ||
  ...
  for (let i = 0; i < 5; i += 1) {
    ...
    while (getTickCount() - tick < 300) {
      if (originalSlot !== me.weaponswitch) {
        return true;
      }
      delay(50);
    }
  ...
  //----iomars
  // !Config.PacketCasting && weaponSwitch(0) && (switched = true);

  // let packetHandler = (bytes) =>
  //   bytes.length > 0 &&
  //   bytes[0] === sdk.packets.recv.WeaponSwitch &&
  //   (switched = true) &&
  //   false; // false to not block
...
me.castingFrames = function (skillId, fcr, charClass) {
  ...
  let isLightning =
    skillId === sdk.skills.Lightning || skillId === sdk.skills.ChainLightning;
  ...
  let animationSpeed = {
    ...
    charClass === sdk.player.class.Druid
      ? me.getState(sdk.states.Wolf) || me.getState(sdk.states.Bear)
      : "normal"
  ];
  return Math.ceil(
    (256 * baseCastRate) /
      Math.floor((animationSpeed * (100 + effectiveFCR)) / 100) -
      (isLightning ? 0 : 1)
  );
};
...
me.getWeaponQuantity = function (weaponLoc = sdk.body.RightArm) {
  let currItem = me
    .getItemsEx(-1, sdk.items.mode.Equipped)
...
Unit.prototype.checkItem = function (itemInfo) {
  ...
  typeof itemObj.name === "number" &&
    (itemObj.name = getLocaleString(itemObj.name));

  let items = this.getItemsEx().filter((item) => {
    return (
      !item.questItem &&
      ...
      (itemObj.basetype === null ||
        (item.normal || item.superior) === itemObj.basetype) &&
      (!itemObj.name ||
        item.fname.toLowerCase().includes(itemObj.name.toLowerCase()))
    );
  });
...
Unit.prototype.findFirst = function (itemInfo = []) {
  ...
  if (!Array.isArray(itemInfo) || typeof itemInfo[0] !== "object")
    return { have: false, item: null };
  ...
  for (let i = 0; i < itemInfo.length; i++) {
    ...
    let items = itemList.filter(function (item) {
      return (
        !item.questItem &&
        (itemObj.classid === -1 || item.classid === itemObj.classid) &&
        (itemObj.itemtype === -1 || item.itemType === itemObj.itemtype) &&
  ...
  switch (typeof id) {
    ...
    case "string":
      if (typeof this.prefixes !== "object") {
        return (
          this.prefix.replace(/\s+/g, "").toLowerCase() ===
      ...
      for (let i = 0; i < this.prefixes.length; i += 1) {
        if (
          id.replace(/\s+/g, "").toLowerCase() ===
          this.prefixes[i].replace(/\s+/g, "").toLowerCase()
        ) {
          return true;
        }
      }
  ...
  switch (typeof id) {
    ...
    case "string":
      if (typeof this.suffixes !== "object") {
        return (
          this.suffix.replace(/\s+/g, "").toLowerCase() ===
          id.replace(/\s+/g, "").toLowerCase()
        );
      }
      ...
      for (let i = 0; i < this.suffixes.length; i += 1) {
        if (
          id.replace(/\s+/g, "").toLowerCase() ===
          this.suffixes[i].replace(/\s+/g, "").toLowerCase()
        ) {
          return true;
        }
      }
...
Unit.prototype.__defineGetter__("dexreq", function () {
  ...
  let finalReq =
    baseReq + Math.floor((baseReq * reqModifier) / 100) - (ethereal ? 10 : 0);

  return Math.max(finalReq, 0);
});

Unit.prototype.__defineGetter__("strreq", function () {
  let ethereal = this.getFlag(sdk.items.flags.Ethereal);
  let reqModifier = this.getStat(sdk.stats.ReqPercent);
...
Unit.prototype.__defineGetter__("itemclass", function () {
  if (getBaseStat("items", this.classid, "code") === undefined) return 0;
  if (
    getBaseStat("items", this.classid, "code") ===
    getBaseStat(0, this.classid, "ultracode")
  )
    return 2;
  if (
    getBaseStat("items", this.classid, "code") ===
    getBaseStat(0, this.classid, "ubercode")
  )
    return 1;
...
Unit.prototype.getStatEx = function (id, subid) {
  ...
  switch (id) {
    case sdk.stats.AllRes: {
      // Block scope due to the variable declaration // calculates all res, doesn't exist though
      // Get all res
      let allres = [
        ...
        for (let i = 0; i < temp.length; i += 1) {
          switch (temp[i][0]) {
            ...
            case id + 2: // plus two handed dmg
              ...
              if (rval) {
                // First occurence stored, return if the second one exists
                return rval;
              }

              if (
                this.getStat(temp[i][0]) > 0 &&
                this.getStat(temp[i][0]) > temp[i][2]
              ) {
    ...
    case sdk.stats.Defense:
      if (subid === 0) {
        ...
        if (this.desc) {
          ...
          regex = new RegExp(
            "\\+\\d+ " +
              getLocaleString(sdk.locale.text.Defense).replace(
                /^\s+|\s+$/g,
                "i"
              )
          );
          ...
          for (let i = 0; i < temp.length; i += 1) {
            if (temp[i].match(regex)) {
              return parseInt(temp[i].omitColor(), 10);
            }
          }
        }
    ...
    case sdk.stats.PoisonMinDamage:
      if (subid === 1) {
        return Math.round(
          (this.getStat(sdk.stats.PoisonMinDamage) *
            this.getStat(sdk.stats.PoisonLength)) /
            256
        );
      }
    ...
    switch (id) {
      case sdk.stats.ArmorPercent:
        ...
        if (this.desc) {
          temp = this.desc.split("\n");
          regex = new RegExp(
            getLocaleString(sdk.locale.text.EnhancedDefense).replace(
              /^\s+|\s+$/g,
              "i"
            )
          );
      ...
      case sdk.stats.EnhancedDamage:
        ...
        if (this.desc) {
          temp = this.desc.split("\n");
          regex = new RegExp(
            getLocaleString(sdk.locale.text.EnhancedDamage).replace(
              /^\s+|\s+$/g,
              "i"
            )
          );
...
Unit.prototype.getColor = function () {
  ...
  if (
    [
      sdk.items.quality.Magic,
      sdk.items.quality.Set,
      sdk.items.quality.Rare,
  ...
  if (
    this.quality === sdk.items.quality.Magic ||
    this.quality === sdk.items.quality.Rare
  ) {
    colors = {
      ...
      for (let i = 0; i < 127; i += 1) {
        if (
          this.fname
            .split("\n")
            .reverse()[0]
            .includes(getLocaleString(getBaseStat(16, i, 3)))
        ) {
    ...
    for (let i = 0; i < 401; i += 1) {
      if (
        this.code === getBaseStat(17, i, 4).replace(/^\s+|\s+$/g, "") &&
        this.fname
          .split("\n")
          .reverse()[0]
          .includes(getLocaleString(getBaseStat(17, i, 2)))
      ) {
...
Unit.prototype.castChargedSkill = function (...args) {
  ...
  switch (args.length) {
    ...
    case 1:
      if (args[0] instanceof Unit) {
        // hellfire.castChargedSkill(monster);
        unit = args[0];
      } else {
        skillId = args[0];
      }
    ...
    case 2:
      if (typeof args[0] === "number") {
        ...
        throw new Error(
          " invalid arguments, expected (skillId, unit) or (x, y)"
        );
      }
  ...
  if (this === me) {
    ...
    this.getItemsEx(-1)
      .filter(
        (item) =>
          item && (item.isEquipped || (item.isInInventory && item.isCharm))
      )
      .forEach(function (item) {
        ...
        if (stats.hasOwnProperty(sdk.stats.ChargedSkill)) {
          if (stats[sdk.stats.ChargedSkill] instanceof Array) {
            ...
            if (
              stats[sdk.stats.ChargedSkill].skill === skillId &&
              stats[sdk.stats.ChargedSkill].charges > 1
            ) {
              chargedItems.push({
    ...
    if (chargedItems.length === 0)
      throw Error(
        "Don't have the charged skill (" + skillId + "), or not enough charges"
      );
    ...
    if (skillId) {
      ...
      charge = charge.filter(
        (item) => skillId && item.skill === skillId && !!item.charges
      );
    ...
    if (charge) {
      ...
      if (
        !Config.PacketCasting ||
        (Config.PacketCasting === 1 && skillId !== sdk.skills.Teleport)
      ) {
        return Skill.cast(
          skillId,
...
Unit.prototype.equip = function (destLocation = undefined) {
  ...
  const findspot = function (item) {
    ...
    return tempspot
      ? { location: Storage.Inventory.location, coord: tempspot }
      : false;
  };
  const doubleHanded = [
    sdk.items.type.Staff,
    sdk.items.type.Bow,
    sdk.items.type.Polearm,
    sdk.items.type.Crossbow,
    sdk.items.type.HandtoHand,
    sdk.items.type.AmazonBow,
    sdk.items.type.AmazonSpear,
  ];

  // Not an item, or unidentified, or not enough stats
  if (
    this.type !== sdk.unittype.Item ||
    !this.getFlag(sdk.items.flags.Identified) ||
  ...
  console.log(
    "equiping " + this.name + " to bodylocation: " + destLocation.first()
  );

  let currentEquiped = me
    .getItemsEx(-1)
    .filter(
      (item) =>
        destLocation.indexOf(item.bodylocation) !== -1 || // Deal with double handed weapons
        (item.isOnMain &&
  ...
  if (!currentEquiped.length) {
    Game.clickItemAndWait(sdk.clicktypes.click.Left, this);
    Game.clickItemAndWait(sdk.clicktypes.click.Left, destLocation.first());
  } else {
    // unequip / swap items
    currentEquiped.forEach((item, index) => {
      ...
      if (index === currentEquiped.length - 1) {
        ...
        Game.clickItemAndWait(sdk.clicktypes.click.Left, this); // Pick up current item
        Game.clickItemAndWait(sdk.clicktypes.click.Left, destLocation.first()); // the swap of items
        // Find a spot for the current item
        let spot = findspot(item);

        if (!spot) {
          // If no spot is found for the item, rollback
      ...
      Game.clickItemAndWait(sdk.clicktypes.click.Left, item.bodylocation);
      Game.clickItemAndWait(
        sdk.clicktypes.click.Left,
        spot.coord.x,
        spot.coord.y,
...
Unit.prototype.getBodyLoc = function () {
  ...
  types[sdk.body.RightArm] = [
    ...
    sdk.items.type.Spear,
    sdk.items.type.Polearm,
    sdk.items.type.Crossbow,
    sdk.items.type.Mace,
    sdk.items.type.ThrowingKnife,
    sdk.items.type.ThrowingAxe,
    sdk.items.type.Javelin,
    sdk.items.type.HandtoHand,
    sdk.items.type.Orb,
    sdk.items.type.AmazonBow,
    sdk.items.type.AmazonSpear,
    sdk.items.type.AmazonJavelin,
    sdk.items.type.AssassinClaw,
  ]; // weapons
  (types[sdk.body.LeftArm] = [
    sdk.items.type.Shield,
...
Unit.prototype.getRes = function (type, difficulty) {
  if (
    !type ||
    ![
      sdk.stats.FireResist,
      sdk.stats.ColdResist,
      sdk.stats.PoisonResist,
      sdk.stats.LightningResist,
    ].includes(type)
  ) {
    return -1;
  }
  ...
  Object.defineProperties(Object.prototype, {
    distance: {
      get: function () {
        return !me.gameReady
          ? NaN
          : /* Math.round */ getDistance.apply(null, [
              me,
              ...coords.apply(this),
            ]);
      },
  ...
  Object.prototype.mobCount = function (givenSettings = {}) {
    ...
    const settings = Object.assign(
      {},
      {
        range: 5,
        coll:
          sdk.collision.BlockWall |
          sdk.collision.LineOfSight |
          sdk.collision.BlockMissile,
        type: 0,
        ignoreClassids: [],
      },
      givenSettings
    );
    return Game.getUnits(sdk.unittype.Monster).filter(function (mon) {
      return (
        mon.attackable &&
        getDistance(x, y, mon.x, mon.y) < settings.range &&
...
Object.defineProperties(Unit.prototype, {
  ...
  isSuperUnique: {
    get: function () {
      return (
        (this.spectype &
          (sdk.monsters.spectype.Super | sdk.monsters.spectype.Unique)) >
        0
      );
    },
  },
  ...
  isPrimeEvil: {
    get: function () {
      return (
        [
          sdk.monsters.Andariel,
          sdk.monsters.Duriel,
          sdk.monsters.Mephisto,
          sdk.monsters.Diablo,
          sdk.monsters.Baal,
          sdk.monsters.BaalClone,
        ...
        getBaseStat("monstats", this.classid, "primeevil")
      );
    },
  },
  isBoss: {
    get: function () {
      return (
        this.isPrimeEvil ||
        [
          sdk.monsters.TheSmith,
          sdk.monsters.BloodRaven,
          sdk.monsters.Radament,
          sdk.monsters.Griswold,
          sdk.monsters.TheSummoner,
          sdk.monsters.Izual,
          sdk.monsters.Hephasto,
          ...
          sdk.monsters.TheCowKing,
          sdk.monsters.ColdwormtheBurrower,
          sdk.monsters.Nihlathak,
        ].includes(this.classid)
      );
    },
  },
  isGhost: {
    get: function () {
      return (
        [
          sdk.monsters.Ghost1,
          sdk.monsters.Wraith1,
          sdk.monsters.Specter1,
          sdk.monsters.Apparition,
          sdk.monsters.DarkShape,
          sdk.monsters.Ghost2,
          sdk.monsters.Wraith2,
          sdk.monsters.Specter2,
        ].includes(this.classid) ||
        getBaseStat("monstats", this.classid, "MonType") ===
          sdk.monsters.type.Wraith
      );
    },
  },
  isDoll: {
    get: function () {
      return [
        sdk.monsters.BoneFetish1,
        sdk.monsters.BoneFetish2,
        sdk.monsters.BoneFetish3,
        sdk.monsters.SoulKiller3,
        sdk.monsters.StygianDoll2,
        sdk.monsters.StygianDoll6,
        sdk.monsters.SoulKiller,
      ].includes(this.classid);
    },
  },
  isMonsterObject: {
    get: function () {
      return [
        sdk.monsters.Turret1,
        sdk.monsters.Turret2,
        sdk.monsters.Turret3,
        sdk.monsters.MummyGenerator,
        sdk.monsters.GargoyleTrap,
        sdk.monsters.LightningSpire,
        sdk.monsters.FireTower,
        sdk.monsters.BarricadeDoor1,
        sdk.monsters.BarricadeDoor2,
        sdk.monsters.BarricadeWall1,
        sdk.monsters.BarricadeWall2,
        sdk.monsters.CatapultS,
        sdk.monsters.CatapultE,
        sdk.monsters.CatapultSiege,
        sdk.monsters.CatapultW,
        sdk.monsters.BarricadeTower,
        sdk.monsters.PrisonDoor,
        sdk.monsters.DiablosBoneCage,
        sdk.monsters.Hut,
      ].includes(this.classid);
    },
  },
  isMonsterEgg: {
    get: function () {
      return [
        sdk.monsters.SandMaggotEgg,
        sdk.monsters.RockWormEgg,
        sdk.monsters.DevourerEgg,
        sdk.monsters.GiantLampreyEgg,
        sdk.monsters.WorldKillerEgg1,
        sdk.monsters.WorldKillerEgg2,
      ].includes(this.classid);
    },
  },
  isMonsterNest: {
    get: function () {
      return [
        sdk.monsters.FoulCrowNest,
        sdk.monsters.BlackVultureNest,
        sdk.monsters.BloodHawkNest,
        sdk.monsters.BloodHookNest,
        sdk.monsters.BloodWingNest,
        sdk.monsters.CloudStalkerNest,
        sdk.monsters.FeederNest,
        sdk.monsters.SuckerNest,
      ].includes(this.classid);
    },
  },
  isBaalTentacle: {
    get: function () {
      return [
        sdk.monsters.Tentacle1,
        sdk.monsters.Tentacle2,
        sdk.monsters.Tentacle3,
        sdk.monsters.Tentacle4,
        sdk.monsters.Tentacle5,
      ].includes(this.classid);
    },
  },
  isShaman: {
    get: function () {
      return [
        ...
        sdk.monsters.CarverShaman2,
        sdk.monsters.DevilkinShaman2,
        sdk.monsters.DarkShaman1,
        sdk.monsters.WarpedShaman,
        sdk.monsters.CarverShaman,
        sdk.monsters.DevilkinShaman,
        sdk.monsters.DarkShaman2,
      ].includes(this.classid);
    },
  },
  isUnraveler: {
  ...
  isFallen: {
    get: function () {
      return [
        sdk.monsters.Fallen,
        sdk.monsters.Carver2,
        sdk.monsters.Devilkin2,
        sdk.monsters.DarkOne1,
        sdk.monsters.WarpedFallen,
        sdk.monsters.Carver1,
        sdk.monsters.Devilkin,
        sdk.monsters.DarkOne2,
      ].includes(this.classid);
    },
  },
  isBeetle: {
  ...
  isWalking: {
    get: function () {
      return (
        this.mode === sdk.monsters.mode.Walking &&
  ...
  isRunning: {
    get: function () {
      return (
        this.mode === sdk.monsters.mode.Running &&
        (this.targetx !== this.x || this.targety !== this.y)
      );
    },
  },
  ...
  isEquippedCharm: {
    get: function () {
      if (this.type !== sdk.unittype.Item) return false;
      return (
        this.location === sdk.storage.Inventory &&
        [
          sdk.items.type.SmallCharm,
          sdk.items.type.LargeCharm,
          sdk.items.type.GrandCharm,
  ...
  isInInventory: {
    get: function () {
      ...
      return (
        this.location === sdk.storage.Inventory &&
        this.mode === sdk.items.mode.inStorage
      );
    },
  },
  isInStash: {
  ...
  isInCube: {
    get: function () {
      if (this.type !== sdk.unittype.Item) return false;
      return (
        this.location === sdk.storage.Cube &&
        this.mode === sdk.items.mode.inStorage
      );
    },
  },
  isInStorage: {
    get: function () {
      ...
      return (
        this.mode === sdk.items.mode.inStorage &&
        [sdk.storage.Inventory, sdk.storage.Cube, sdk.storage.Stash].includes(
          this.location
        )
      );
    },
  ...
  isInBelt: {
    get: function () {
      if (this.type !== sdk.unittype.Item) return false;
      return (
        this.location === sdk.storage.Belt &&
        this.mode === sdk.items.mode.inBelt
      );
    },
  },
  isOnMain: {
    get: function () {
      if (
        this.type !== sdk.unittype.Item ||
        this.location !== sdk.storage.Equipped
      )
        return false;
    ...
    get: function () {
      ...
      return (
        (me.weaponswitch === sdk.player.slot.Main &&
          [sdk.body.RightArmSecondary, sdk.body.LeftArmSecondary].includes(
            this.bodylocation
          )) ||
        (me.weaponswitch === sdk.player.slot.Secondary &&
          [sdk.body.RightArm, sdk.body.LeftArm].includes(this.bodylocation))
      );
    },
  },
  identified: {
  ...
  questItem: {
    get: function () {
      if (this.type !== sdk.unittype.Item) return false;
      return (
        this.itemType === sdk.items.type.Quest ||
        [
          sdk.items.quest.HoradricMalus,
          sdk.items.quest.WirtsLeg,
          sdk.items.quest.HoradricStaff,
          sdk.items.quest.ShaftoftheHoradricStaff,
          sdk.items.quest.ViperAmulet,
          sdk.items.quest.DecoyGidbinn,
          sdk.items.quest.TheGidbinn,
          sdk.items.quest.KhalimsFlail,
          sdk.items.quest.KhalimsWill,
          sdk.items.quest.HellForgeHammer,
          sdk.items.quest.StandardofHeroes,
        ].includes(this.classid)
      );
    },
  },
    ...
    get: function () {
      ...
      return (
        ...
        [
          ...
          sdk.items.quest.TokenofAbsolution,
          sdk.items.quest.TwistedEssenceofSuffering,
          sdk.items.quest.ChargedEssenceofHatred,
          sdk.items.quest.BurningEssenceofTerror,
  ...
  onGroundOrDropping: {
    get: function () {
      if (this.type !== sdk.unittype.Item) return false;
      return (
        this.mode === sdk.items.mode.onGround ||
        this.mode === sdk.items.mode.Dropping
      );
    },
  },
  isShield: {
...
//iomars
Unit.prototype.getItemSkills = function (isSingle = true) {
  if (this.type !== sdk.unittype.Item) return undefined;

  let stats = this.getStat(-1);
  if (stats === undefined) return undefined;

  return stats.filter(
    (item) =>
      item[0] === (isSingle ? sdk.stats.SingleSkill : sdk.stats.AddSkillTab)
  );
};
...
Object.defineProperties(me, {
  ...
  moving: {
    get: function () {
      return [
        sdk.player.mode.Walking,
        sdk.player.mode.Running,
        sdk.player.mode.WalkingInTown,
      ].includes(me.mode);
    },
  },
  highestAct: {
    ...
    get: function () {
      for (let i = sdk.quest.id.Respec; i >= sdk.quest.id.SpokeToWarriv; i--) {
        ...
        if (
          ...
          me.getQuest(i, sdk.quest.states.ReqComplete)
        ) {
          return i;
        }
      }
  ...
  staminaDrainPerSec: {
    get: function () {
      ...
      return (
        (25 *
          Math.max(
            (40 * (1 + armorMalusReduction / 10) * (100 - bonusReduction)) /
              100,
            1
          )) /
        256
  ...
  shapeshifted: {
    get: function () {
      return (
        me.getState(sdk.states.Wolf) ||
        me.getState(sdk.states.Bear) ||
        me.getState(sdk.states.Delerium)
      );
    },
  },
  mpPercent: {
  ...
  bloodraven: {
    get: function () {
      return me.getQuest(
        sdk.quest.id.SistersBurialGrounds,
  ...
  gidbinn: {
    get: function () {
      return me.getQuest(
        sdk.quest.id.BladeoftheOldReligion,
        sdk.quest.states.Completed
      );
    },
  },
  ...
  torchKey: {
    get: function () {
      return [
        ...
        let items = me.findItems(item, sdk.items.mode.inStorage);
...
Unit.prototype.__defineGetter__("attackable", function () {
  ...
  if (
    this.type === sdk.unittype.Player &&
    getPlayerFlag(me.gid, this.gid, 8) &&
    !this.dead
  )
    return true;
  // Dead monster
  ...
  if (
    [
      sdk.monsters.CarrionBird1,
      sdk.monsters.UndeadScavenger,
      sdk.monsters.HellBuzzard,
      sdk.monsters.WingedNightmare,
      sdk.monsters.SoulKiller2 /*feel like this one is wrong*/,
      sdk.monsters.CarrionBird2,
    ].includes(this.classid) &&
    this.mode === sdk.monsters.mode.UsingSkill1
  ) {
    return false;
  }
  ...
  if (
    [
      sdk.monsters.SandMaggot,
      sdk.monsters.RockWorm,
      sdk.monsters.Devourer,
      sdk.monsters.GiantLamprey,
      sdk.monsters.WorldKiller2,
      sdk.monsters.WaterWatcherLimb,
      sdk.monsters.RiverStalkerLimb,
      sdk.monsters.StygianWatcherLimb,
      sdk.monsters.WaterWatcherHead,
      sdk.monsters.RiverStalkerHead,
      sdk.monsters.StygianWatcherHead,
    ].includes(this.classid) &&
    this.mode === sdk.monsters.mode.Spawning
  ) {
    return false;
  }

  return (
    [sdk.monsters.ThroneBaal, sdk.monsters.Cow /*an evil force*/].indexOf(
      this.classid
    ) === -1
  );
});
...
Unit.prototype.__defineGetter__("curseable", function () {
  ...
  if (
    !!this.name &&
    !!this.name.includes(getLocaleString(sdk.locale.text.Possessed))
  )
    return false;
  if (
    this.type === sdk.unittype.Player &&
...
Unit.prototype.__defineGetter__("scareable", function () {
  return (
    this.curseable &&
    !this.isSpecial &&
    this.classid !== sdk.monsters.ListerTheTormenter
  );
});

Unit.prototype.getMobCount = function (
  range = 10,
  coll = 0,
  type = 0,
  noSpecialMobs = false
...
Unit.prototype.checkForMobs = function (givenSettings = {}) {
  ...
  if (mob) {
    do {
      if (
        getDistance(_this, mob) < settings.range &&
        mob.attackable &&
 ...
 * @description replicate .? operator of modern js since d2boot doesn't have it
 */
