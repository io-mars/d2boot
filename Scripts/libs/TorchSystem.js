...
export const TorchSystem = {
  ...
  getFarmers: function () {
    ...
    for (let i in this.FarmerProfiles) {
      if (this.FarmerProfiles.hasOwnProperty(i)) {
        for (
          let j = 0;
          j < this.FarmerProfiles[i].KeyFinderProfiles.length;
          j += 1
        ) {
          if (
            this.FarmerProfiles[i].KeyFinderProfiles[j].toLowerCase() ===
  ...
  inGameCheck: function () {
    ...
    for (let i = 0; i < farmers.length; i += 1) {
      if (
        farmers[i].FarmGame.length > 0 &&
        me.gamename.match(new RegExp(farmers[i].FarmGame, "i"))
      ) {
        print("\xFFc4Torch System\xFFc0: In Farm game.");
        D2Bot.printToConsole(
          "Torch System: Transfering keys.",
  ...
  keyCheck: function () {
                  ...
                  if (item) {
                    do {
                      if (item.code === "rvs" || item.code === "rvl") {
                        ...
                        if (
                          neededItems[i].length >= Math.min(2, obj.value[i])
                        ) {
                          break;
                        }
                      }
    ...
    for (let i = 0; i < farmers.length; i += 1) {
      sendCopyData(
        ...
        farmers[i].profile,
        6,
        JSON.stringify({ name: "keyCheck", profile: me.profile })
      );
      delay(250);

      if (
        neededItems.hasOwnProperty("pk1") ||
  ...
  outOfGameCheck: function () {
        ...
        if (obj && obj.name === "gameName") {
          for (let i = 0; i < farmers.length; i += 1) {
            if (
              obj.value.gameName
                .toLowerCase()
                .match(farmers[i].FarmGame.toLowerCase())
            ) {
    ...
    for (let i = 0; i < farmers.length; i += 1) {
      sendCopyData(
        ...
        farmers[i].profile,
        6,
        JSON.stringify({ name: "gameCheck", profile: me.profile })
      );
      delay(500);
  ...
  waitForKeys: function () {
              ...
              case "keyCheck":
                if (farmer.KeyFinderProfiles.includes(obj.profile)) {
                  ...
                  sendCopyData(
                    ...
                    obj.profile,
                    6,
                    JSON.stringify({ name: "neededItems", value: neededItems })
                  );
                }
    ...
    while (true) {
      ...
      if (
        ((tkeys >= 3 && hkeys >= 3 && dkeys >= 3) ||
          (Config.OrgTorch.WaitTimeout &&
            getTickCount() - timer >
              Config.OrgTorch.WaitTimeout * 1000 * 60)) &&
        this.aloneInGame()
      ) {
