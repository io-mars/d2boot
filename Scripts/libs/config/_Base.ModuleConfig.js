...
import { JoinSettings } from "@/config/_StarterConfig.js";
...
export const BaseConfig = function () {
  ...
  Config.Countess.ClearLastTower = true;
  ...
  Scripts.MFTorches = false; // Runs Pandemonium by team.
  Config.MFTorches.MaxQuantity = 5;
  Config.MFTorches.UseSalvation = false;
  Config.MFTorches.MuleProfile = false; // the profile of charm mule
  ...
  Config.BaalHelper.Town = false; // back town if run others script after baal
  ...
  Config.Follower.Picker = false; //pick item
  Config.Follower.PickGold = false; //pick gold
  Config.Follower.SwitchAct = false; //auto switch act
  ...
  Config.HealMP = 60; // Go to a healer if under designated percent of mana.
  ...
  Config.PickitFiles.push("default.nip");
  ...
  Config.ClearPath = {
    // Areas: [sdk.areas.ArcaneSanctuary], // Specific areas to clear while traveling in. Comment out to clear in all areas
    //iomars if set [], then clear all
    Areas: [sdk.areas.None], // Specific areas to clear while traveling in. Comment out to clear in all areas
  ...
  ]; // List of monster NAMES or CLASSIDS to static. Example: Config.StaticList = ["Andariel", 243];  Config.UseTelekinesis = true; // Use telekinesis on units that allow it. Example: Shrines, Waypoints, Chests, and Portals
  ...
  Config.Recipes.push([Recipe.Socket.Weapon, "Giant Thresher", Roll.Eth]); // Socket ethereal Giant Thresher
  Config.Recipes.push([Recipe.Socket.Weapon, "Cryptic Axe", Roll.Eth]); // Socket ethereal Cryptic Axe

  Config.Recipes.push([Recipe.Socket.Weapon, "Grand Matron Bow", Roll.NonEth]); // Socket Grand Matron Bow
  Config.Recipes.push([Recipe.Socket.Weapon, "Matriarchal Bow", Roll.NonEth]); // Socket Matriarchal Bow
  ...
  Config.Recipes.push([Recipe.Socket.Shield, "Monarch", Roll.All]); // Socket Monarch shield
  ...
  Config.Recipes.push([Recipe.Blood.Gloves, "Vampirebone Gloves", Roll.NonEth]); // Craft Blood Gloves
  ...
  ////Config.Recipes.push([Recipe.Caster.Gloves, "Bramble Mitts", Roll.NonEth]); // Craft Caster Gloves
  ...
  Config.Recipes.push([Recipe.HitPower.Gloves, "Vambraces", Roll.NonEth]); // Craft Hit Power Gloves
  ...
  Config.Recipes.push([Recipe.Reroll.Rare, "Diadem", Roll.NonEth]); // Reroll rare Diadem

  // the cubing formula: 1 Perfect Skull + 1 Rare Item + Stone of Jordan = 1 high quality new rare item of the same type
  // Config.Recipes.push([Recipe.Reroll.HighRare, "Diadem", Roll.NonEth]); // Reroll high rare Diadem
  ...
  Config.KeepRunewords.push(
    "[type] == polearm # [meditationaura] == 17 && [enhanceddamage] >= 250"
  );
  // perfect Insight Level 12-17 Meditation Aura 200-260% Enhanced Damage (varies) 180-250% Bonus to Attack Rating (varies)
  //Config.KeepRunewords.push("[type] == polearm # [meditationaura] == 17 && [enhanceddamage] >= 260 && [attackrate] >= 250"); // perfect Insight

  Config.Runewords.push([Runeword.Obedience, "Great Poleaxe", Roll.Eth]); // Make Obedience Great Poleaxe
  Config.Runewords.push([Runeword.Obedience, "Giant Thresher", Roll.Eth]); // Make Obedience Giant Thresher
  Config.Runewords.push([Runeword.Obedience, "Cryptic Axe", Roll.Eth]); // Make Obedience Cryptic Axe
  //All Resistances +20-30 (varies)
  Config.KeepRunewords.push(
    "[type] == polearm # [enhanceddamage] >= 370 && [fireresist] >= 25"
  );
  Config.KeepRunewords.push(
  ...
  // Config.KeepRunewords.push(
  //   "[name] == crystalsword || [name] == phaseblade || [name] == flail # [plusskillbattlecommand] >= 6 && [plusskillbattleorders] >=6 && [plusskillbattlecry] >= 4"
  // ); // perfect CTA and *optional [enhanceddamage] = 290%
  ...
  Config.Runewords.push([Runeword.Harmony, "Diamond Bow"]); // Make Harmony
  Config.Runewords.push([Runeword.Harmony, "Crusader Bow"]); // Make Harmony
  Config.Runewords.push([Runeword.Harmony, "Ward Bow"]); // Make Harmony
  Config.Runewords.push([Runeword.Harmony, "Hydra Bow"]); // Make Harmony
  Config.Runewords.push([Runeword.Harmony, "Grand Matron Bow"]); // Make Harmony
  Config.Runewords.push([Runeword.Harmony, "Matriarchal Bow"]); // Make Harmony

  Config.KeepRunewords.push("[type] == bow # [enhanceddamage] >= 270"); //Harmony
  Config.KeepRunewords.push("[type] == amazonbow # [enhanceddamage] >= 270"); //Harmony
...
export const Follower = function () {
  Config.PickitFiles.push("newbie.nip");
...
export const MFHelperLeader = function () {
  ...
  Config.Countess.ClearLastTower = false;
  Config.Countess.KillGhosts = false; //
  ...
  Scripts.MFTorches = true; // Runs Pandemonium by MF team.
  Config.MFTorches.MuleProfile = "charmMuler"; // the profile of charm mule
...
export const RunewordMaker = function () {
  //add runeword pickit file
  // Config.PickitFiles.push("team-rw.nip");
  ...
  Config.Runewords.push([Runeword.Insight, "Giant Thresher", Roll.Eth]); // Make Insight Giant Thresher
  //Level 12-17 Meditation Aura When Equipped (varies) +200-260% Enhanced Damage (varies) 180-250% Bonus to Attack Rating (varies) +1-6 To Critical Strike (varies)
  Config.KeepRunewords.push(
    "[type] == polearm # [meditationaura] == 17 && [enhanceddamage] >= 250"
  );
  ...
  Config.Runewords.push([Runeword.Spirit, "Monarch", Roll.NonEth]); // Monarch
  Config.Runewords.push([Runeword.Spirit, "Sacred Targe", Roll.NonEth]); // Sacred Targe

  Config.KeepRunewords.push(
    "[type] == shield || [type] == auricshields # [fcr] >= 35"
  );
};
...
export const ItemUpgrader = function () {
  Config.Cubing = true; // Set to true to enable cubing.
  ...
  //
  // Config.Recipes.push([Recipe.Unique.Armor.ToElite, "War Boots", Roll.NonEth]); // Upgrade Gore Rider to Elite
  // Config.Recipes.push([Recipe.Unique.Armor.ToElite, "Serpentskin Armor", Roll.NonEth]); // Upgrade serpentskin armor to Elite
  // Config.Recipes.push([
  //   Recipe.Unique.Weapon.ToElite,
  //   "Ceremonial Javelin",
  //   Roll.All,
  // ]); // Upgrade ceremonialjavelin
  Config.Recipes.push([
    Recipe.Unique.Armor.ToElite,
    "Gilded Shield",
    Roll.NonEth,
  ]); //Upgrade gildedshield to Elite
...
export const CharmCuber = function () {
  Config.Cubing = true; // Set to true to enable cubing.

  //----------------------
  //!!!NOTE!!!: clear all Recipes, fix the pgem bug on cubing gemList
  //----------------------
  ...
  // will locked PG if set here!!
  // Config.Recipes.push([Recipe.Socket.Weapon, "Berserker Axe", Roll.Eth]); // Socket ethereal Thresher
  // Config.Recipes.push([Recipe.Socket.Weapon, "Thresher", Roll.Eth]); // Socket ethereal Thresher
  // Config.Recipes.push([Recipe.Socket.Weapon, "Great Poleaxe", Roll.Eth]); // Socket ethereal Great Poleaxe
  // Config.Recipes.push([Recipe.Socket.Weapon, "Giant Thresher", Roll.Eth]); // Socket ethereal Giant Thresher
  // Config.Recipes.push([Recipe.Socket.Weapon, "Cryptic Axe", Roll.Eth]); // Socket ethereal Cryptic Axe
  ...
  // Config.Recipes.push([Recipe.Socket.Armor, "Archon Plate", Roll.NonEth]); // Socket ethereal Archon Plate

  Config.Recipes.push([Recipe.Reroll.Magic, "Grand Charm"]); // Reroll magic Grand Charm (ilvl 95+)
  Config.Recipes.push([Recipe.Reroll.Rare, "Diadem", Roll.NonEth]); // Reroll rare Diadem
};

export const Enchanter = function () {
  Config.QuitTimeout = 5000;
  Config.PublicMode = 2; // 1 = invite and accept, 2 = accept only, 3 = invite only, 0 = disable.

  Scripts.ControlBot = true;
  Config.ControlBot.RequestLimited = false; // request limited
  Config.ControlBot.RushHelper = true; // request limited
  Config.ControlBot.Bo = true; // Bo player at waypoint
  Config.ControlBot.Cows.MakeCows = true; // allow making cows if we can
  Config.ControlBot.Cows.GetLeg = true; // Get Wirt's Leg from Tristram. If set to false, it will check for the leg in town.
  Config.ControlBot.Chant.Enchant = true; // enchant player and their minions on command
  ...
  Config.ControlBot.Wps.SecurePortal = true; // Secure wp before making portal
  Config.ControlBot.EndMessage = ""; // Message before quitting
  Config.ControlBot.GameLength = 60; // Game length in minutes

  Scripts.Rusher = false; // Rush bot. For a list of commands, see Rusher.js
  Config.Rusher.WaitPlayerCount = 0; // Wait until game has a certain number of players (0 - don't wait, 8 - wait for full game).
  Config.Rusher.Sequence = false; // Do quest one by one of sequence
  Config.Rusher.Cain = true; // Do cain quest.
  ...
  Config.Rusher.HellAncients = true; // Does Ancient's quest in hell (only if quester is level 60+)
  Config.Rusher.GiveWps = true; // Give all Wps
  Config.Rusher.LastRun = ""; // End rush after this run.
};
...
export const AutoBuilder = function () {
  ...
  //	logs activity to /logs/AutoBuild.CharacterName._MM_DD_YYYY.log
  //	It automatically enables Config.AutoBuild.Verbose
  Config.AutoBuild.Template = "nothing"; //	The name of the build associated with an existing
};
