...
export const Mephisto = function () {
    ...
    while (attackCount < 300 && meph.attackable(meph)) {
      if (meph.mode === sdk.monsters.mode.Attacking2) {
        ...
        for (let i = 0; i < angles.length; i += 1) {
          pos.dist = 18;
          pos.x = Math.round(
            Math.cos(((angle + angles[i]) * Math.PI) / 180) * pos.dist + meph.x
          );
          pos.y = Math.round(
            Math.sin(((angle + angles[i]) * Math.PI) / 180) * pos.dist + meph.y
          );
  ...
  this.killCouncil = function () {
    ...
    for (let i = 0; i < coords.length; i += 2) {
      ...
      Attack.clearList(
        Attack.getMob(
          [sdk.monsters.Council1, sdk.monsters.Council2, sdk.monsters.Council3],
          0,
          40
        )
      );
  ...
  if (Config.Mephisto.TakeRedPortal) {
    ...
    while (
      getCollision(me.area, 17601, 8070, 17590, 8068) !== 0 &&
