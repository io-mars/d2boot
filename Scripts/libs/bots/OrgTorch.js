...
export const OrgTorch = function () {
  ...
  this.completeSetCheck = function () {
    ...
    return (
      horns.length === brains.length &&
      horns.length === eyes.length &&
      brains.length === eyes.length
    );
  };
  ...
  this.getFade = function () {
    if (
      ...
      console.log(
        sdk.colors.Orange + "OrgTorch :: " + sdk.colors.White + "Getting Fade"
      );
      ...
      console.log(
        sdk.colors.Orange + "OrgTorch :: " + sdk.colors.Green + "Fade Achieved"
      );
    }
  ...
  this.matronsDen = function () {
    ...
    return (
      me.findItems(sdk.items.quest.DiablosHorn, sdk.items.mode.inStorage)
        .length > dHorns
    );
  };

  this.forgottenSands = function () {
    let bEyes = me.findItems(
    ...
    try {
      for (let i = 0; i < nodes.length; i++) {
        ...
        let eye = Game.getItem(
          sdk.items.quest.BaalsEye,
          sdk.items.mode.onGround
        );
    ...
    return (
      me.findItems(sdk.items.quest.BaalsEye, sdk.items.mode.inStorage).length >
      bEyes
    );
  };

  this.furnance = function () {
    let mBrain = me.findItems(
    ...
    return (
      me.findItems(sdk.items.quest.MephistosBrain, sdk.items.mode.inStorage)
        .length > mBrain
    );
  };
  ...
  this.uberTrist = function () {
    let skillBackup;
    let useSalvation =
      Config.OrgTorch.UseSalvation && Skill.canUse(sdk.skills.Salvation);
  ...
  let redPortals = getUnits(sdk.unittype.Object, sdk.objects.RedPortal).filter(
    (el) =>
      [
        sdk.areas.MatronsDen,
        sdk.areas.ForgottenSands,
  ...
  if (redPortals.length > 0) {
    ...
    this.currentGameInfo.doneAreas.length > 0 &&
      (this.currentGameInfo = OrgTorchData.create());
  }

  // End the script if we don't have enough keys nor organs
  if (
    (tkeys < keySetsReq || hkeys < keySetsReq || dkeys < keySetsReq) &&
