...
void Profile::init(const wchar_t *profileName)
{
  ...
  GetPrivateProfileString(profileName, L"mode", L"single", mode, _countof(mode), file);
  GetPrivateProfileString(profileName, L"character", L"ERROR", charname, _countof(charname), file);
  GetPrivateProfileString(profileName, L"spdifficulty", L"0", difficulty, _countof(difficulty), file);
  GetPrivateProfileString(profileName, L"username", L"ERROR", username, _countof(username), file);
  GetPrivateProfileString(profileName, L"password", L"ERROR", password, _countof(password), file);
  GetPrivateProfileString(profileName, L"gateway", L"ERROR", gateway, _countof(gateway), file);

  GetPrivateProfileString(L"settings", L"MaxLoginTime", L"5", _maxLoginTime, _countof(_maxLoginTime), file);
  GetPrivateProfileString(L"settings", L"MaxCharSelectTime", L"5", _maxCharTime, _countof(_maxCharTime), file);
  ...
  while (!loginComplete)
  {
    ...
    case OOG_ENTER_IP_ADDRESS:
      if (_wcsicmp(ip, L""))
      {
        ...
        errorMsg = "Could not get the IP address from the profile in the D2Boot.ini file.";
